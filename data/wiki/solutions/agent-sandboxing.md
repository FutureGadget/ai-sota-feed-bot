...
## What's new
The commoditized base layer gains a self-hostable hypervisor: Tarit is an
open-source, rust-vmm-based microVM hypervisor built for AI-agent and RL
workloads, offered as a self-hosted alternative to Firecracker for teams
that want execution-sandbox isolation without a managed cloud platform.

That builds on the harness-level default (Claude Code's "Manual" permission
mode), independent output verification (SonarQube plugins across Claude
Code, Copilot, Codex, and Cursor), the network-perimeter control (Google
Cloud VPC Service Controls), operated platforms (Grab's Palana), and the
existing drop-in layer (Workdir, Cerberus).

...
when the automated judge is in doubt. Hamel Husain's own FAQ, distilled from
teaching 700+ engineers and PMs, packages the same lesson as a standing
reference rather than a one-off audit: it argues teams should spend
60-80% of eval development time on manual error analysis — reading at
least 100 diverse traces before automating anything — and warns that a
100% pass rate is itself a signal the eval suite isn't challenging the
system, not a result to celebrate.
...
A thirty-first front sources benchmark tasks from **production telemetry
instead of hand-written or synthetic scenarios**: BTS-AgentBench turns
read-only industrial telemetry logs into executable multi-turn agent
episodes through a deterministic, replayable pipeline — normalize
metadata, build a read-only tool store, compile tasks with gold answers
and evidence, then bound them into typed episodes — with quality gates
(contract preflight reports, timestamp policies, evidence attribution) and
reproducibility checked by independent rebuilds matching all 11 logical
tool-store exports. Applied to one industrial system (XAI4HEAT) it produced
204 episodes from real operational logs, extending this page's "the
benchmark's own construction is a variable" thread (the serving-backend and
container-resourcing fronts above) to how the tasks themselves get built,
not just how they get run.

A thirty-second front answers the standing benchmark-contamination problem
with a cryptographic separation rather than a trust agreement: Google
DeepMind's double-blind evaluation pilot runs a model provider and an
external evaluator inside Confidential Computing (Google Cloud's
Confidential Space) so the evaluator never sees the model weights and the
provider never sees the evaluation prompts, with both sides able to verify
the separation held via cryptographic evidence instead of a policy promise.
Piloted with Singapore's AI Safety Institute, OpenMined, AVERI, and
MLCommons against a Gemini Flash Lite model, it targets the specific bias
this page's "familiar benchmarks over-state capability" thread has assumed
was procedural (a provider could always "peek" at eval questions in
advance) and makes it structurally impossible instead of merely
discouraged.

A thirty-third front questions the standard method for evaluating **model
switching** itself, not the model or the benchmark: static replay evaluation
swaps a different model's output into a logged trajectory at the switch
point and assumes the rest of the trajectory is unaffected. Branching-rollout
tests on live SWE-bench trajectories (~900 rollouts, forking the environment
at a controlled point and continuing with a different model per fork) show
that assumption fails hard: post-fork actions diverge from the logged path
61-94% of the time, replay mispredicts every success-relevant outcome, and
patch similarity to what actually happened falls to 0.00-0.11. All five
observed success/failure flips occurred in swap forks and zero occurred
across 359 same-model control forks, so the divergence is a genuine effect of
the swap, not sampling noise — and even temperature-0 "determinism" turned
out to depend on quantization, with FP8 controls diverging on over 90% of
forks while AWQ stayed near-identical. The fix isn't a different scoring
rubric but a different evaluation architecture: fork and re-run the
environment per model instead of splicing one model's output into another's
logged trajectory, extending this page's "the benchmark's own construction is
a variable" thread (BTS-AgentBench, the serving-backend and
...
A thirty-fifth front pairs **simulation-driven testing** with the standing
trajectory-versus-outcome split rather than adding a new metric: a
practitioner account (Columbia's Zhou Yu, with Arklex AI) argues agents stall
in demo phase because teams grade the final answer and skip the
multi-turn path that produced it, and proposes synthetic user personas plus
trajectory entropy as pre-deployment signals, wired into automated CI/CD
pipelines that catch edge cases before release rather than after — the same
process-over-outcome argument this page already makes (trajectory/process
evaluation, AgentOps), here framed as a build discipline for scaling
self-learning agent workflows rather than a benchmark result.

A thirty-sixth front sharpens hallucination-detection evaluation itself into
a **multi-annotator** discipline rather than a single-pass label: treating
factual-error detection in long-form chatbot responses as a one-shot,
single-annotator labeling problem misses errors that are subtle and embedded
within mostly-correct text, so a multi-perspective annotation protocol scores
agreement across several independent judgments per response instead of
trusting one grader's call — the same "the judge itself needs auditing"
lesson BabelJudge and Hamel Husain's 100-trace comparison already establish
on this page, here applied to the specific failure mode of catching a subtle
factual error rather than a gross one.

A thirty-seventh front turns the scoring pipeline itself into the
unreliable variable, sharpening the standing "familiar benchmarks over-state
what they measure" critique into a magnitude nobody had quantified: an
audit of eight cybersecurity LLM benchmarks across 10 proprietary,
open-weight, and cybersecurity-specialized models finds a single pipeline
choice — how a response is parsed, scored, or normalized, not the task
itself — can swing a model's score by more than 80 percentage points and
change which model comes out ahead. Two benchmarks measuring the same
underlying skill rank the same 10 models differently because their scoring
conventions are incompatible, and standardizing pipeline choices while
holding task semantics fixed still shifts nine of the ten models by at
least three ranks on at least one benchmark. It is a specific, measured
instance of the same "container config alone swings scores 6 points"
finding this page already tracks on Terminal-Bench 2.0 — except an order of
magnitude larger, and located in how a benchmark grades an answer rather
than the compute environment it runs in.

A thirty-eighth front turns the eval-to-deployment handoff itself into the
thing under scrutiny, not the eval or the model: production agents drift
from what was evaluated through routine, individually-reasonable changes —
prompt edits, tool-schema updates, policy shifts, knowledge-base refreshes,
model fallbacks, memory effects — so evaluating well is necessary but is not
authorization to deploy. The proposed fix names a **behavioral identity**:
track an agent through its whole lifecycle by an identifiable revision
rather than a convenient label like `customer-support-v4` that describes
intent, not observed behavior, then chain seven stages — declare, evaluate,
certify, authorize, execute, observe, audit — under that one identity so a
team can answer "why was this agent allowed to do this." It's the same
agent-as-identity argument [prompt injection](/topic/prompt-injection)
already makes for authorization and credentials, applied one layer earlier:
to whether the thing that was certified is provably the thing now running,
not just what it's allowed to touch.

A named production platform adds a build-vs-buy instance to the standing
production-ROI thread (Motorway's AWS pipeline, LangChain's Harbor
overhaul): Grab's LLM-Kit standardizes secret handling, runtime tool
discovery, and evaluation integration across more than 500 internal agent
services, and reports cutting the time to deploy a new agent from roughly
two weeks to about one hour. It's the deployment-path counterpart to Grab's
own security-focused Palana platform (see [agent
sandboxing](/topic/agent-sandboxing)) — a paved-road framework that bakes
evaluation in from the start rather than leaving each new agent to assemble
it alone.

A named production pattern sharpens this page's eval/observability
convergence (already tracked via AgentOps) with a two-layer split rather
than one shared trace substrate: AWS pairs Amazon Bedrock AgentCore
Evaluations (continuous quality scoring) with a separate AWS DevOps Agent
(autonomous infrastructure investigation) to monitor a production
multi-agent system, on the premise that multi-agent failures slip past
traditional single-agent monitoring — demonstrated on a four-agent airline
reservation system. It's a concrete production instance of this page's
standing "the unit under test is widening from a single agent to the whole
harness" argument, applied to live monitoring rather than a pre-deployment
benchmark.

Third-party evaluation is also gaining a formal governance standard rather
than staying an ad hoc arrangement between a lab and an outside auditor: the
AI Evaluator Forum's AEF-1 baseline standard sets criteria for access,
conflicts of interest, funding relationships, recusal, and transparency, and
xAI, OpenAI, and Anthropic have all cosigned it. Anthropic's own commitment
is the most concrete piece of the standard: giving embedded third-party
evaluators (such as METR) desks, access badges, and company laptops, with
permissions comparable to an internal risk-assessment team — evaluators
positioned inside the company like a banking regulator, continuously
assessing both shipped models and training pipelines, rather than auditing
from outside after the fact. It sharpens this page's standing eval-governance
thread — the behavioral-identity trust chain, Google DeepMind's double-blind
evaluation pilot — with a cross-lab institutional commitment rather than one
company's mechanism.

Discovering the right benchmark is itself becoming a tooling problem, not
just building one: Benchmark Radar is a living database and search engine
for AI benchmarks — covering LLM evaluation, agentic and tool-use
benchmarks, coding, reasoning, safety, and domain-specific suites —
combining daily discovery of new benchmark papers with retrieval over what
already exists, so a team choosing among the ever-widening domain-narrow
list this page tracks (Rails, AWS-Bench, VLoc Bench, and dozens more) does
not have to track every new suite by hand.

A thirty-ninth front pits a non-LLM evaluator architecture directly against
LLM-as-judge on the axes that matter for production grading, not just
accuracy: LangChain tested TypeSafe AI's Jev — now shipping as a judge
inside LangSmith Evals — against LLM judges on accuracy, repeatability,
latency, and cost, positioning it as a faster, cheaper option for scoring
agent traces at the volume production evals and regression tests need. It
extends this page's standing "the judge itself is under audit" thread
(BabelJudge) with a competing evaluator architecture, not only a validation
method for the LLM-judge default.

The domain-narrow benchmark list keeps widening into infrastructure and
regulated verticals. Databricks names branch isolation and a serverless
architecture among the criteria for evaluating a database built specifically
for AI agents, and Handshake's Atlas-Finance benchmark evaluates agents
operating inside a bank — both extending this page's eval-target coverage
into domain- and infrastructure-specific ground rather than general-purpose
tool use. A companion question sharpens what "evaluation" means for a
stateful capability rather than a one-shot answer: whether an agent's memory
is still current, not just whether it was once correct, ties this page's
...
A forty-first front turns evaluation itself into an autonomous, self-healing
loop rather than a human-triggered audit: LangSmith's Engine v2 mines
production traces and repos to generate hypotheses about issues that
haven't surfaced yet, tests those hypotheses, and surfaces confirmed
failures for review — then, for agents on managed deployment, re-runs the
offending inputs to confirm the issue and tests candidate fixes against a
broader eval set until one resolves it, opening a PR on request. It widens
what counts as an eval signal past errors and unmet requests to performance
trends (error rate, latency, cost) and inefficient trajectories (repetitive
tool calls, unnecessarily long runs), automating the diagnose-fix-validate
loop this page's standing "data-mining problem, not a labeling exercise"
framing (LangChain's own practice, Langy) already argues for, one step
further into the fix itself.

The domain-narrow benchmark list widens into **production inference
serving** specifically: NVIDIA's SWE-Serve turns real SGLang engineering
work — model integration, public APIs, cache systems, GPU kernels — into 53
tasks qualified against reference solutions and regression tests, scored
Pass@1 across 31 model/harness configurations (three runs each) in
environments with blocked public web access. Reported pass rates range
20.8-75.5% across models, extending this page's harness-and-infrastructure
benchmark coverage ([agent latency](/topic/agent-latency)) into the
inference-serving codebase itself rather than a general coding task.

## What's new
LangSmith's Engine v2 automates its own fix loop — hypothesis generation
from production traces, confirmation re-runs, and candidate-fix validation
against a broader eval set — and NVIDIA's SWE-Serve adds a 53-task,
Pass@1-scored benchmark for agentic engineering inside a real inference-serving
codebase (SGLang), widening the domain-narrow benchmark list into production
serving specifically (see State of the art above).

Prior update: A joint UK AISI/EvalEval effort targets benchmark-result reproducibility

...
## What's new
A fourth wave of the Claude Agent SDK's one-line-changelog pattern is the
largest yet: four releases (v0.2.135, v0.2.136, v0.2.138, v0.2.139) each read
only "updated bundled Claude CLI," but three of the four CLI versions they
forward carry undisclosed security fixes — v2.1.227 alone fixes four
permission-check bypasses, v2.1.232 fixes a PowerShell and a Windows Git Bash
bypass, and v2.1.233 closes an NTLM credential-leak vector. A new open-source
tool, Drift, is now framing this exact obstacle — behavior shifting between

...
## What's new
MACE frames shared multi-agent memory as a co-evolution problem: functional
memory units scored and organized from task outcomes, not just written and
read, averaging 81.11% across eight benchmarks against ten baselines (see
State of the art above).

Prior update: LinkedIn's tiered-memory reference architecture now has a named
product behind it: Contextual Agent Playbooks and Tools serve procedural
memory and code search over MCP specifically to overcome agent limitations
in large codebases.

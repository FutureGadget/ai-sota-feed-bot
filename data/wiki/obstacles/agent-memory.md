...
A companion angle targets the **cost of the evolution loop itself**, not
what the evolved skills unlock: COBRA-Skills frames skill improvement as
budgeted sequential optimization over an evolving candidate pool, using
contextual-bandit-guided prioritization to steer expensive execution-based
evaluation toward the most promising candidates instead of scoring every
one equally. Across six benchmarks and three target models it holds
top-tier performance while cutting optimization cost 55-58% versus a
SkillOpt baseline and needing only 50 examples per benchmark — the same
skill-evolution idea Recuris demonstrates above, aimed at making the loop
cheap enough to run routinely rather than at what running it buys.
...
The local-first roster gets a spec-backed entrant rather than a one-off
implementation: OKF Agent Memory stores facts, decisions, and domain
concepts as git-tracked Markdown with YAML frontmatter, implementing
Google's Open Knowledge Format (OKF) v0.2 — a normative structure with
provenance, trust tiers, and lifecycle metadata baked in, not just a file
convention. Retrieval is in-memory BM25 with no vector database and no
embedding-API cost, returning results in sub-300 microseconds, and a
progressive-disclosure index lets an agent pull only the specific concept it
needs instead of loading the whole corpus, cutting token consumption
roughly 80% versus dumping the full knowledge base into context. It's the
same zero-LLM-retrieval, developer-owned pattern this page already tracks
(Sibyl, PMB, Awareness Local), now built to an external, vendor-neutral spec
instead of a project's own schema.

The local-first roster also picks up a **temporal-graph** entrant that
narrows the recall verb itself: Fraise is a single-binary memory database
that stores facts as a temporal graph of facts, topics, and entities, queried
through exactly two verbs — `remember` to write and `recall` to read — with
recall ranked and capped rather than exhaustive, on the explicit reasoning
that the caller pays for every token a recall reads back. It's the same
cost-conscious, deterministic-write instinct this page already tracks
(FERNme's Hebbian tags, PMB's BM25-plus-vector index), applied to the shape
of the query interface rather than the storage format.

A narrower entrant answers coding-agent state loss with **explicit consent**
rather than automatic capture: SOS stores a defined "latest state" of a
project inside the repo and serves it to a new or resumed session over MCP,
but the state must be explicitly saved and accepted — nothing is inferred or
filled in by guesswork, and a superseded result stays in history without
continuing to influence the agent's future trajectory. It's a stricter write
discipline than the automatic-capture stores above, aimed at the same
...
Coordination between agents writing to *shared* memory now has a dedicated
learning framework, not just a locking scheme: MACE (Memory-Agent
Co-Evolution) organizes a multi-agent system's collaboration traces into
functional memory units — grouped conditions, actions, and outputs linked
by support, conflict, and repair relations — then selects which units and
formats (instructions vs. checklists) to hand each agent within a memory
budget, and updates unit scores from task outcomes rather than scoring
retrieval and presentation separately. Across eight benchmarks it averages
81.11%, beating ten baselines — evidence that *how* a shared memory store is
organized and presented back to agents, not just whether writes conflict, is
its own tunable variable (see [multi-agent](/topic/multi-agent) for the
coordination side of the same shared-memory problem).

Institutional memory is also showing up as a packaged product rather than a
DIY architecture: OpenAI's V7, built on GPT-5.6, turns a company's scattered
files into context agents can pull from to complete complex, source-linked
work — the enterprise-knowledge-as-memory pattern this page already tracks
(LinkedIn's Contextual Agent Playbooks, Elastic's Atlas), this time shipped
as a standalone agent product rather than an internal platform layer.

The **isolation** question above (per-user memory, not just per-tenant) gets
a mainstream-framework answer: LangChain's Managed Deep Agents 0.8 ships a
two-tier memory config — `define_memory(agent=MemoryLayer(), user=MemoryLayer())`
— where user-scoped memory mounts at `/memories/user/`, keys to the
authenticated caller's identity, and is allowed in one-to-one Slack DMs but
denied by default in group chats and HTTP channels, so one user's context
can't leak into another's conversation through a shared agent deployment. It
pairs the memory split with per-user credential scoping (OAuth across 23
services) so both what an agent remembers and what it's authorized to touch
follow the caller, not just the deployment. A hardware-based answer to the
same "remember across sessions without leaking between users" problem comes
from the infrastructure layer instead of the application layer: Google's
Private AI Compute runs server-side memory inside hardware-enforced secure
enclaves, unlockable only with device-held encryption keys, so persistent
context survives between sessions without the provider itself being able to
read it — trading LangChain's per-user application scoping for a
privacy-by-construction guarantee at the infrastructure level.

The measurability push above (Agent Memory Leaderboard, the failure-modes
and UTILMEM benchmarks) picks up a dedicated frontier-mapping benchmark
rather than a single-number leaderboard entry: DolphinBench scores agent
memory systems specifically on real-world, action-dependent long-term
recall — the kind an agent needs to take a correct later action, not the
conversational-QA recall most memory benchmarks were built for — and maps
the resulting Pareto frontier of accuracy against cost/latency rather than
picking one winner. A companion architecture answers the *how* question
the benchmark measures: AutoViewMem self-configures orthogonal views over
conversational history — instead of a single fixed index, it maintains
several complementary projections of the same stored memory and picks
which view to query per request, the same "recall depends on which
interface you query, not just what's stored" argument CABLE already makes
on this page. A companion practitioner signal reinforces the same push
from outside the benchmark literature: "Beyond Context Windows" argues
evaluating long-term memory has to look past context-window size entirely,
the same framing this page's tiered-store and brute-force-context threads
above already contest from the architecture side.

The local-first coding-agent memory wave keeps widening with three more
grassroots entrants in the same short window: Memanto.ai and an
open-source shared-memory project for teams of AI coding agents both
target the same cross-agent, cross-session recall gap as Sibyl and
Awareness Local above, while Captain Memo bundles memory alongside skills
and capabilities into one local package any coding agent can use — the
same "own it, don't rent it" instinct as the rest of the local-first
roster, this time packaging memory and skills together rather than memory
alone.

## What's new
LangChain's Managed Deep Agents 0.8 adds per-user memory isolation
(`/memories/user/`, keyed to caller identity, denied by default outside 1:1
DMs) alongside per-user credential scoping, and Google's Private AI Compute
answers the same cross-session-without-leakage problem at the infrastructure
layer with hardware-enclave-based server-side memory (see State of the art
above).

Prior update: DolphinBench maps the Pareto frontier of agent memory on action-dependent
long-term recall rather than conversational QA, and AutoViewMem answers
the same measurement with a self-configuring, multi-view retrieval
architecture instead of one fixed index (see State of the art above).

Prior update: MACE frames shared multi-agent memory as a co-evolution problem: functional
memory units scored and organized from task outcomes, not just written and
read, averaging 81.11% across eight benchmarks against ten baselines
(cross-ref [multi-agent](/topic/multi-agent) for the coordination side of
the same shared-memory problem).

# Daily AI SOTA Digest - 2026-07-28

Focus: AI Platform Engineering

## 1. An opinionated guide to which AI to use to do stuff
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/27/an-opinionated-guide-to-which-ai-to-use-to-do-stuff/#atom-everything
- Score: 2.633 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, codex.

## 2. Evaluating Agents Beyond the First Prompt
- Type: news | Source: philschmid
- URL: https://www.philschmid.de/evocode-bench
- Score: 2.626 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 3. Full Text Search in SmithDB: Designing an Inverted Index for Object Storage
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/full-text-search-in-smithdb-designing-an-inverted-index-for-object-storage
- Score: 2.547 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 4. The harness is all you need (mostly)
- Type: news | Source: github_blog_ai_ml
- URL: https://github.blog/ai-and-ml/github-copilot/the-harness-is-all-you-need-mostly/
- Score: 2.519 | Reliability: 1.0
- Why it matters: Matches feed focus: harness.

## 5. GitHub Copilot app for Beginners: Getting started
- Type: news | Source: github_blog_ai_ml
- URL: https://github.blog/ai-and-ml/github-copilot/github-copilot-app-for-beginners-getting-started/
- Score: 2.478 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 6. moonshotai/Kimi-K3
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/27/kimi-k3/#atom-everything
- Score: 2.477 | Reliability: 1.0
- Why it matters: 

## 7. Our position on open-weights models
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/position-open-weights-models
- Score: 2.404 | Reliability: 1.0
- Why it matters: 

## 8. Kimi K3 weights released: China's most powerful agentic AI from Moonshot AI free - Notebookcheck
- Type: release | Source: search_cn_open_weight_labs
- URL: https://news.google.?oc=5
- Score: 2.347 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 9. How AI is expanding what people do at work
- Type: news | Source: openai_blog
- URL: https://openai.com/index/how-ai-is-expanding-what-people-do-at-work
- Score: 2.187 | Reliability: 1.0
- Why it matters: 

## 10. Expanding our partnership with Cognizant | Anthropic
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/cognizant-anthropic
- Score: 2.16 | Reliability: 1.0
- Why it matters: 

## 11. Article: An Evolutionary Architecture Pattern for Managing AI’s Pace of Change
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/articles/evolutionary-architecture-pattern/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.346 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 12. vllm v0.26.0
- Type: release | Source: vllm_releases
- URL: https://github.com/vllm-project/vllm/releases/tag/v0.26.0
- Score: 2.328 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 13. Own Your Intelligence: The Key to Lasting AI Advantage
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/own-your-intelligence
- Score: 2.269 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 14. codex 0.146.0-alpha.13
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.146.0-alpha.13
- Score: 2.184 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 15. Where Is the Cost of Third-Party API Routers in Agentic Software Development?
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2607.23624v1
- Score: 2.086 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation.

## 16. NVIDIA Cosmos-H-Dreams: Bringing Real-Time Generative Simulation to Surgical Robotics
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/nvidia/cosmos-h-dreams
- Score: 1.893 | Reliability: 1.0
- Why it matters: 

## 17. The Regression Tax: Decomposing Why Skills Help and Hurt LLM Agents
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2607.22520v1
- Score: 1.876 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, harness, evaluation.

## 18. Project Pilot: Can AI models fly drones?
- Type: research | Source: anthropic_research
- URL: https://www.anthropic.com/research/project-pilot
- Score: 1.712 | Reliability: 1.0
- Why it matters: 

## 19. Netflix Details Its In-House LLM Serving Platform with Triton and vLLM - infoq.com
- Type: news | Source: search_llm_ops_news
- URL: https://news.google.com/rss/articles/CBMiZ0FVX3lxTFBpby0wOTVaSTc0LWVRam9WelpQRDdhdW1KMGJYWFRIRWZnQXFGYWdhVjAxUGRhNkRlRGl4VEZnRFVnbEVqeWJtdGVqSFlNa1BIcVhtZzBqdFZHTHlQS3daQXdJRmdrQ0U?oc=5
- Score: 1.69 | Reliability: 1.0
- Why it matters: 

## 20. SLAI T-Rex: Full-Parameter Post-training of the DeepSeek-V4 Family on Ascend SuperPOD
- Type: paper | Source: arxiv_cn_open_weight_reports
- URL: http://arxiv.org/abs/2607.20145v1
- Score: 1.428 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 21. Beyond RAG: Task-aware knowledge compression for enterprise AI on AWS
- Type: news | Source: aws_ml_blog
- URL: https://aws.amazon.com/blogs/machine-learning/beyond-rag-task-aware-knowledge-compression-for-enterprise-ai-on-aws/
- Score: 1.407 | Reliability: 1.0
- Why it matters:

# Daily AI SOTA Digest - 2026-08-25

Focus: AI Platform Engineering

## 1. Microsoft Moves AI Governance From Policy to Runtime Enforcement
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/08/microsoft-ai-governance/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.812 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 2. SWE Refactor Bench: Can Coding Agents Complete a Long-Horizon, Whole-Repository Stack Migration?
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2608.23564v1
- Score: 2.84 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation.

## 3. EvoWiki: Incremental State Overwriting and Traceable Question Answering for Cross-Meeting Knowledge Evolution
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2608.23265v1
- Score: 2.684 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 4. llm-anthropic 0.27
- Type: release | Source: simon_willison
- URL: https://simonwillison.net/2026/Aug/24/llm-anthropic/
- Score: 2.617 | Reliability: 1.0
- Why it matters: Matches feed focus: claude code.

## 5. RAD: Rule-Augmented Relational Anomaly Detection
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2608.23468v1
- Score: 2.592 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 6. [AINews] Andrew Ng gets into AI Engineering
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/ainews-andrew-ng-gets-into-ai-engineering
- Score: 2.574 | Reliability: 1.0
- Why it matters: 

## 7. QiAnXin Discloses Critical Remote-Code-Execution Flaw in DeepSeek Harness
- Type: news | Source: search_cn_open_weight_labs
- URL: https://news.google.com/rss/articles/CBMiiAFBVV95cUxPWTZaeFBaRTBLY0xPcHRiSXZLQl9kTVV3MWluRW4ta0xYVnV3ak5XZWNBZGZ6eEd3VG1UOFY3Y1hKc1dqd0duM0ZjVkhieHdNT0htSmJYaWhtT002Wk5fWVhKQjBSWVFYMUd4SkRiSWhwTU1wSHhwTERHUXNLbHVFeE5ZWUtzR2Yx?oc=5
- Score: 2.55 | Reliability: 1.0
- Why it matters: Matches feed focus: harness.

## 8. Toyota Scales Enterprise AI with Deep Agents and LangSmith
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/how-toyota-north-america-put-enterprise-ai-on-the-balance-sheet-with-deep-agents-and-langsmith
- Score: 2.505 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 9. Your executable is a SQLite database
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Aug/24/your-executable-is-a-sqlite-database/
- Score: 2.493 | Reliability: 1.0
- Why it matters: 

## 10. Advancing price-performance for developers with GPT‑5.6 in Kiro
- Type: news | Source: openai_blog
- URL: https://openai.com/index/gpt-5-6-in-kiro
- Score: 2.233 | Reliability: 1.0
- Why it matters: 

## 11. EarthVerse: Benchmarking Scientific Agents Across Dynamic Earth Systems and Natural Hazards
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2608.23525v1
- Score: 2.485 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 12. We gave every agent a computer
- Type: news | Source: hackernews_ai
- URL: https://onecli.sh/blog/why-we-gave-every-agent-a-computer
- Score: 2.474 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 13. Podcast: The Human Edge: Why Brownfield Codebases Need Mob Programming, Not Just AI Vibes
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/podcasts/brownfield-codebases-mob-programming/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.316 | Reliability: 1.0
- Why it matters: Matches feed focus: claude code.

## 14. codex 0.150.0-alpha.8
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.150.0-alpha.8
- Score: 2.297 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 15. Building a 100x Cheaper Trace Judge with Fireworks
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/building-a-100x-cheaper-trace-judge-with-fireworks
- Score: 2.236 | Reliability: 1.0
- Why it matters: 

## 16. v0.28.0: [CI/Build] Pin Cython below 3.3 for arm64 tilelang sdist (#53358)
- Type: release | Source: vllm_releases
- URL: https://github.com/vllm-project/vllm/releases/tag/v0.28.0
- Score: 2.202 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 17. claude-code v2.1.243
- Type: release | Source: claude_code_releases
- URL: https://github.com/anthropics/claude-code/releases/tag/v2.1.243
- Score: 2.138 | Reliability: 1.0
- Why it matters: 

## 18. Wire It, Run It, Deploy It: AI Workflows in Gradio
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/gradio-workflow-guide
- Score: 2.06 | Reliability: 1.0
- Why it matters: 

## 19. Democratizing institutional knowledge: Building an AI-powered knowledge management system with AWS
- Type: news | Source: aws_ml_blog
- URL: https://aws.amazon.com/blogs/machine-learning/democratizing-institutional-knowledge-building-an-ai-powered-knowledge-management-system-with-aws/
- Score: 1.621 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

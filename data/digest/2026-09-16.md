# Daily AI SOTA Digest - 2026-09-16

Focus: AI Platform Engineering

## 1. Claude Cowork and chat are now one Claude
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Sep/16/one-claude/
- Score: 2.752 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, codex, claude code.

## 2. Reimagining advertising with AI
- Type: news | Source: openai_blog
- URL: https://openai.com/index/reimagining-advertising-with-ai
- Score: 2.675 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 3. How to connect AI usage to business value
- Type: news | Source: openai_blog
- URL: https://openai.com/index/how-to-connect-ai-usage-to-business-value
- Score: 2.511 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 4. Underwriting Superintelligence: Backing Agents you can Sue — Rune Kvist, AIUC
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/aiuc
- Score: 2.479 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 5. Our framework for reporting model misalignment
- Type: news | Source: openai_blog
- URL: https://openai.com/index/model-misalignment-reporting-framework
- Score: 2.46 | Reliability: 1.0
- Why it matters: 

## 6. Helping older adults use AI in everyday life
- Type: news | Source: openai_blog
- URL: https://openai.com/index/helping-older-adults-use-ai-in-everyday-life
- Score: 2.435 | Reliability: 1.0
- Why it matters: 

## 7. ChatGPT vs Gemini vs DeepSeek: 37x Free Context Gap [2026]
- Type: news | Source: search_cn_open_weight_labs
- URL: https://news.google.com/rss/articles/CBMifkFVX3lxTE1hS3dweFItWmZpaGlCZGdoejR4QjM4UXFndnhXUkpHcU1zOFVKNzYzWG1VTVM5ZTdJaURvV3RyeVpzYktJaERyS25BM3BaOXZ4ajdRZDlxZGxucWNQTTVCWXRLcGxGYUxJTlpqS2RTd3BXU0x5ell3Z2pDNXI2Zw?oc=5
- Score: 2.407 | Reliability: 1.0
- Why it matters: 

## 8. codex rust-v0.155.0-alpha.14
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.155.0-alpha.14
- Score: 2.404 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 9. FluxVLA Engine: A One-Stop VLA Engineering Platform for Embodied Intelligence
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2609.17210v1
- Score: 2.402 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 10. Quoting Mustafa Suleyman
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Sep/16/mustafa-suleyman/
- Score: 2.357 | Reliability: 1.0
- Why it matters: 

## 11. Article: Your Next DSL Author Is a Language Model
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/articles/next-dsl-author-language-model/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.339 | Reliability: 1.0
- Why it matters: 

## 12. [AINews] Jev: a “System One Model” that only decides/classifies/routes/scores — >100x faster, >200x cheaper than small frontier LLMs
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/ainews-jev-a-system-one-model-that
- Score: 2.314 | Reliability: 1.0
- Why it matters: 

## 13. claude-code v2.1.273
- Type: release | Source: claude_code_releases
- URL: https://github.com/anthropics/claude-code/releases/tag/v2.1.273
- Score: 2.311 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval, claude code.

## 14. Dropbox Outlines How Focusing on Existing Infrastructure Efficiency Can Create Headroom for AI
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/09/dropbox-datacenter/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.308 | Reliability: 1.0
- Why it matters: 

## 15. ScienceBuddy: Recursive-in-Recursive Self-Improvement for Interactive Scientific Agents
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2609.17523v1
- Score: 2.258 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, harness, evaluation.

## 16. Introducing Gemini 3.8 Live and 3.8 Live Extended Thinking
- Type: news | Source: google_deepmind_blog
- URL: https://deepmind.google/blog/introducing-gemini-3-8-live-and-3-8-live-extended-thinking/
- Score: 2.159 | Reliability: 1.0
- Why it matters: 

## 17. Cloud CISO Perspectives: How Google monitors AI threats and advances AI defenses
- Type: news | Source: google_cloud_blog
- URL: https://cloud.google.com/blog/products/identity-security/cloud-ciso-perspectives-how-google-monitors-ai-threats-advances-ai-defenses/
- Score: 2.134 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, harness, codex.

## 18. claude-agent-sdk-python v0.2.153
- Type: release | Source: claude_agent_sdk_python_releases
- URL: https://github.com/anthropics/claude-agent-sdk-python/releases/tag/v0.2.153
- Score: 2.109 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 19. Scaling Agents in Europe & The Middle East: Lessons from Schneider Electric, Vodafone, and monday.com
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/scaling-agents-in-europe-the-middle-east-lessons-from-schneider-electric-vodafone-and-monday-com
- Score: 2.099 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 20. Claude Cowork and chat are now one Claude | Claude by Anthropic
- Type: news | Source: claude_blog
- URL: https://claude.com/blog/cowork-is-now-claude
- Score: 2.036 | Reliability: 1.0
- Why it matters: 

## 21. Your Agent Aced the Task. Will It Do It Again?
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/ibm-research/altk-evolve-consistency
- Score: 2.032 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 22. IRENE: A Convolutional GRU Ensemble Model for Radar Precipitation Nowcasting over Italy
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2609.17175v1
- Score: 1.942 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 23. When scanners miss the attack: how Cloudflare Client-Side Security protects storefronts
- Type: news | Source: cloudflare_blog
- URL: https://blog.cloudflare.com/client-side-security-finds-4-malicious-campaigns/
- Score: 1.872 | Reliability: 1.0
- Why it matters: 

## 24. Claude for Small Business launches new workflows, integrations, and training programs | Claude by Anthropic
- Type: news | Source: claude_blog
- URL: https://claude.com/blog/claude-for-small-business-launches-new-workflows-integrations-and-training-programs
- Score: 1.804 | Reliability: 1.0
- Why it matters:

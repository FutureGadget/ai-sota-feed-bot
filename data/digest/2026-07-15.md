# Daily AI SOTA Digest - 2026-07-15

Focus: AI Platform Engineering

## 1. How to Debug Coding Agents with LangSmith Traces
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/your-coding-agents-are-a-black-box-heres-how-to-crack-them-open
- Score: 2.892 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, codex, claude code.

## 2. Legal AI, not a coding agent with scaffolding
- Type: news | Source: hackernews_ai
- URL: https://lexifina.com/blog/legal-ai-not-a-coding-agent-with-scaffolding
- Score: 2.695 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 3. 5 Trends That Defined AI Engineering at World’s Fair 2026
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/aiewf26trends
- Score: 2.625 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 4. simonw/pedalican
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/14/pedalican/#atom-everything
- Score: 2.624 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, codex.

## 5. Can LLMs Write Reliable Rubrics? A Meta-Evaluation for Experiment Reproduction
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2607.12835v1
- Score: 2.589 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 6. Label-Decoupled Style Augmentation for Domain Generalization in Multi-Label Remote Sensing Scene Classification
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2607.12704v1
- Score: 2.578 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 7. Do AI Agents Know When a Task Is Simple? Toward Complexity-Aware Reasoning and Execution
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2607.13034v1
- Score: 2.577 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, harness, eval.

## 8. Introducing Claude for Teachers
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/claude-for-teachers
- Score: 2.426 | Reliability: 1.0
- Why it matters: 

## 9. How to manage AI investments in the agentic era
- Type: news | Source: openai_blog
- URL: https://openai.com/index/managing-ai-investments-in-agentic-era
- Score: 2.42 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 10. Anthropic commits $10 million to Canadian AI research
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/canadian-ai-research
- Score: 2.248 | Reliability: 1.0
- Why it matters: 

## 11. Google and Industry Partners Announce Agentic Resource Discovery Specification for AI Agents
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/07/agentic-resource-discovery-spec/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.549 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 12. Google Cloud Workbench Notebooks Extension Connects VS Code to Google Cloud's Jupyter Notebooks
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/07/cloud-workbench-vscode-extension/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.539 | Reliability: 1.0
- Why it matters: 

## 13. Quoting Armin Ronacher
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/14/armin-ronacher/#atom-everything
- Score: 2.53 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 14. codex 0.145.0-alpha.12
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.145.0-alpha.12
- Score: 2.402 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 15. claude-agent-sdk-python v0.2.119
- Type: release | Source: claude_agent_sdk_python_releases
- URL: https://github.com/anthropics/claude-agent-sdk-python/releases/tag/v0.2.119
- Score: 2.364 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 16. Resist and Update: Counterfactual Report Coordinates for Incentive-Compatible LLMs
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2607.12985v1
- Score: 2.346 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 17. How Canada uses Claude
- Type: research | Source: anthropic_research
- URL: https://www.anthropic.com/research/how-canada-uses-claude
- Score: 2.327 | Reliability: 1.0
- Why it matters: 

## 18. claude-code v2.1.210
- Type: release | Source: claude_code_releases
- URL: https://github.com/anthropics/claude-code/releases/tag/v2.1.210
- Score: 2.165 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, claude code.

## 19. vllm v0.25.1
- Type: release | Source: vllm_releases
- URL: https://github.com/vllm-project/vllm/releases/tag/v0.25.1
- Score: 1.785 | Reliability: 1.0
- Why it matters:

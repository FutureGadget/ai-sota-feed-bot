# Daily AI SOTA Digest - 2026-06-19

Focus: AI Platform Engineering

## 1. Datasette Apps: Host custom HTML applications inside Datasette
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jun/18/datasette-apps/#atom-everything
- Score: 3.551 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation, codex.

## 2. AutoPass: Evidence-Guided LLM Agents for Compiler Performance Tuning
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2606.20373v1
- Score: 3.262 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 3. Execution-State Capsules: Graph-Bound Execution-State Checkpoint and Restore for Low-Latency, Small-Batch, On-Device Physical-AI Serving
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2606.20537v1
- Score: 2.965 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 4. Azure Functions Ships Serverless Agents Runtime at Build 2026
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/06/azure-functions-serverless-agent/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.867 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 5. Windows Platform Security and the Race to Secure AI Agents
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/06/windows-security-agents/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.864 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 6. Improving health intelligence in ChatGPT
- Type: news | Source: openai_blog
- URL: https://openai.com/index/improving-health-intelligence-in-chatgpt
- Score: 2.823 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 7. A Systematic Evaluation of Black-Box Uncertainty Estimation Methods for Large Language Models
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2606.19868v1
- Score: 2.8 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 8. New usage analytics and updated spend controls for enterprises
- Type: news | Source: openai_blog
- URL: https://openai.com/index/chatgpt-enterprise-spend-controls
- Score: 2.795 | Reliability: 1.0
- Why it matters: 

## 9. Claude Code now supports artifacts | Claude
- Type: news | Source: claude_blog
- URL: https://claude.com/blog/artifacts-in-claude-code
- Score: 2.79 | Reliability: 1.0
- Why it matters: Matches feed focus: claude code.

## 10. Steering Claude Code: skills, hooks, subagents and more | Claude
- Type: news | Source: claude_blog
- URL: https://claude.com/blog/steering-claude-code-skills-hooks-rules-subagents-and-more
- Score: 2.79 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, claude code.

## 11. Using AI to help physicians diagnose rare genetic diseases affecting children
- Type: news | Source: openai_blog
- URL: https://openai.com/index/diagnose-rare-childhood-diseases
- Score: 2.778 | Reliability: 1.0
- Why it matters: 

## 12. Is it agentic enough? Benchmarking open models on your own tooling
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/is-it-agentic-enough
- Score: 2.762 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 13. Project Fetch: Phase two
- Type: research | Source: anthropic_research
- URL: https://www.anthropic.com/research/project-fetch-phase-two
- Score: 2.617 | Reliability: 1.0
- Why it matters: 

## 14. datasette-acl 0.6a0
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jun/18/datasette-acl/#atom-everything
- Score: 2.615 | Reliability: 1.0
- Why it matters: 

## 15. Centrally manage authorization for MCP connectors | Claude
- Type: news | Source: claude_blog
- URL: https://claude.com/blog/enterprise-managed-auth
- Score: 2.59 | Reliability: 1.0
- Why it matters: 

## 16. [AINews] GLM > GPT? GLM-5.2 passes vibe check; Z.ai forecasts Open Fable by December
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/ainews-glm-gpt-glm-52-passes-vibe
- Score: 2.531 | Reliability: 1.0
- Why it matters: 

## 17. langgraph==1.2.6
- Type: release | Source: langgraph_releases
- URL: https://github.com/langchain-ai/langgraph/releases/tag/1.2.6
- Score: 2.46 | Reliability: 1.0
- Why it matters: 

## 18. codex 0.142.0-alpha.4
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.142.0-alpha.4
- Score: 2.419 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 19. claude-agent-sdk-python v0.2.105
- Type: release | Source: claude_agent_sdk_python_releases
- URL: https://github.com/anthropics/claude-agent-sdk-python/releases/tag/v0.2.105
- Score: 2.392 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 20. claude-code v2.1.183
- Type: release | Source: claude_code_releases
- URL: https://github.com/anthropics/claude-code/releases/tag/v2.1.183
- Score: 2.37 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 21. Monitor and debug generative AI inference with SageMaker detailed metrics and Insights dashboard on CloudWatch
- Type: news | Source: aws_ml_blog
- URL: https://aws.amazon.com/blogs/machine-learning/monitor-and-debug-generative-ai-inference-with-sagemaker-detailed-metrics-and-insights-dashboard-on-cloudwatch/
- Score: 1.364 | Reliability: 1.0
- Why it matters:

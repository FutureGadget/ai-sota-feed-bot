# Daily AI SOTA Digest - 2026-02-17

Focus: AI Platform Engineering

## 1. Show HN: MCP Codebase Index – 87% fewer tokens when AI navigates your codebase
- Type: news | Source: hackernews_ai
- URL: https://github.com/MikeRecognex/mcp-codebase-index
- Score: 3.936 | Reliability: 1.0
- Why it matters: Direct context efficiency win for agentic coding; measurable metrics; MCP integration; pip-installable; works with Claude Code immediately.

## 2. Qwen3.5: Towards Native Multimodal Agents
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Feb/17/qwen35/#atom-everything
- Score: 2.791 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 3. Two new Showboat tools: Chartroom and datasette-showboat
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Feb/17/chartroom-and-datasette-showboat/#atom-everything
- Score: 2.736 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 4. Agoda’s API Agent Converts Any API to MCP with Zero Code and Deployments
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/02/agoda-api-agent/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.244 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 5. Hunt Globally: Deep Research AI Agents for Drug Asset Scouting in Investing, Business Development, and Search & Evaluation
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2602.15019v1
- Score: 2.404 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 6. Anthropic Rwanda Mou
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/anthropic-rwanda-mou
- Score: 1.86 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 7. Anthropic Infosys
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/anthropic-infosys
- Score: 1.855 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 8. India Brief Economic Index
- Type: research | Source: anthropic_research
- URL: https://www.anthropic.com/research/india-brief-economic-index
- Score: 2.236 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 9. [AINews] Qwen3.5-397B-A17B: the smallest Open-Opus class, very efficient model
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/ainews-qwen35-397b-a17b-the-smallest
- Score: 2.0 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 10. Use What You Know: Causal Foundation Models with Partial Graphs
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2602.14972v1
- Score: 2.19 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 11. v0.1.37
- Type: release | Source: claude_agent_sdk_python_releases
- URL: https://github.com/anthropics/claude-agent-sdk-python/releases/tag/v0.1.37
- Score: 1.841 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 12. rust-v0.102.0-alpha.10
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.102.0-alpha.10
- Score: 1.685 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 13. v2.1.41
- Type: release | Source: claude_code_releases
- URL: https://github.com/anthropics/claude-code/releases/tag/v2.1.41
- Score: 1.65 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

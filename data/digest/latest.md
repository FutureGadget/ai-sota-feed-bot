# Daily AI SOTA Digest - 2026-07-22

Focus: AI Platform Engineering

## 1. A Fireside Chat with Cat and Thariq from the Claude Code team
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/21/cat-and-thariq/#atom-everything
- Score: 3.309 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation, claude code.

## 2. codex 0.145.0
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.145.0
- Score: 3.178 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, codex, claude code.

## 3. A simple API for offering your coding agent a smoke break
- Type: news | Source: hackernews_ai
- URL: https://smoke-break.pineapplefreefall.com
- Score: 2.746 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 4. Graph-Based Agentic AI with LangGraph: Workflow Pathways for Long-Running Stateful Business Processes
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2607.19297v1
- Score: 2.711 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, eval.

## 5. Trace voice agents in LangSmith
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/trace-voice-agents-in-langsmith
- Score: 2.662 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 6. Agents in the Wild: Where Research Meets Deployment
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2607.19336v1
- Score: 2.599 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation.

## 7. Inference-Time Steering for Cross-Lingual Factual Consistency in LLMs
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2607.19243v1
- Score: 2.59 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 8. Introducing the ChatGPT for small business program
- Type: news | Source: openai_blog
- URL: https://openai.com/index/introducing-chatgpt-small-business-program
- Score: 2.49 | Reliability: 1.0
- Why it matters: 

## 9. Introducing Gemini 3.6 Flash, 3.5 Flash-Lite, and 3.5 Flash Cyber
- Type: news | Source: google_deepmind_blog
- URL: https://deepmind.google/blog/introducing-gemini-36-flash-35-flash-lite-and-35-flash-cyber/
- Score: 2.435 | Reliability: 1.0
- Why it matters: 

## 10. OpenAI and Hugging Face partner to address security incident during model evaluation
- Type: news | Source: openai_blog
- URL: https://openai.com/index/hugging-face-model-evaluation-security-incident
- Score: 2.3 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 11. How Apollo Uses Deep Agents and LangSmith for GTM AI
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/how-apollo-rebuilt-its-ai-assistant-on-deep-agents-to-power-the-full-gtm-loop
- Score: 2.518 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 12. Staypoint Detection from Noisy Trajectory Data [Experiment Paper]
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2607.19312v1
- Score: 2.501 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 13. [AINews] AI Cybersecurity becomes top of mind
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/ainews-ai-cybersecurity-becomes-top
- Score: 2.434 | Reliability: 1.0
- Why it matters: 

## 14. A 'millennial genius' China's internet can't get enough of: Moonshot AI's founder - AOL.com
- Type: news | Source: search_cn_open_weight_labs
- URL: https://news.google.com/rss/articles/CBMiigFBVV95cUxPSFBnUVY2OVhiUXQ1LVI2R2xlZ0hZZk1sbnpzaWZnbWNTY1RPTUtwUGdaOUd4THhyWlQ3R3Q1NTlHQThLd0hYVmloNXF1RG11eTU0aXNZSjlfQWUyNGlQb1VITEZiUXdRNTdMdjFscnNRWGthOTNfNWl0NnNSV3ZUZHV4TVFHN2NTZWc?oc=5
- Score: 2.426 | Reliability: 1.0
- Why it matters: 

## 15. claude-code v2.1.217
- Type: release | Source: claude_code_releases
- URL: https://github.com/anthropics/claude-code/releases/tag/v2.1.217
- Score: 2.405 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 16. Nativ: Run AI models locally on your Mac
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/21/nativ/#atom-everything
- Score: 2.365 | Reliability: 1.0
- Why it matters: 

## 17. claude-agent-sdk-python v0.2.125
- Type: release | Source: claude_agent_sdk_python_releases
- URL: https://github.com/anthropics/claude-agent-sdk-python/releases/tag/v0.2.125
- Score: 2.281 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 18. Yelp Unifies ML Model Training with Training Orchestrator
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/07/yelp-ai-model-training/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.242 | Reliability: 1.0
- Why it matters: 

## 19. Why AI apps fail in production (And how Google solved it)
- Type: news | Source: google_cloud_blog
- URL: https://cloud.google.com/blog/topics/developers-practitioners/why-ai-apps-fail-in-production/
- Score: 2.188 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 20. Presentation: Engineering AI for Creativity and Curiosity on Mobile
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/presentations/ai-mobile/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.107 | Reliability: 1.0
- Why it matters: 

## 21. How to build interactive experiences with canvases
- Type: news | Source: github_blog_ai_ml
- URL: https://github.blog/ai-and-ml/github-copilot/how-to-build-interactive-experiences-with-canvases/
- Score: 2.091 | Reliability: 1.0
- Why it matters: 

## 22. The State of Simulation for Physical AI: An Overview
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/nvidia/state-of-simulation-for-physical-ai
- Score: 2.035 | Reliability: 1.0
- Why it matters: 

## 23. Built in Fort Worth: Wistron Opens Advanced Manufacturing Plant to Produce NVIDIA AI Systems
- Type: news | Source: nvidia_blog
- URL: https://blogs.nvidia.com/blog/wistron-manufacturing-texas/
- Score: 1.489 | Reliability: 1.0
- Why it matters:

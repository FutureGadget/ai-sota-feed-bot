# Daily AI SOTA Digest - 2026-07-17

Focus: AI Platform Engineering

## 1. 13 hands-on demos to build on Gemini Enterprise Agent Platform
- Type: news | Source: google_cloud_blog
- URL: https://cloud.google.com/blog/products/ai-machine-learning/13-demos-on-gemini-enterprise-agent-platform/
- Score: 3.026 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation, codex.

## 2. Kimi K3, and what we can still learn from the pelican benchmark
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/16/kimi-k3/#atom-everything
- Score: 3.019 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation.

## 3. QCon AI Boston: Production AI Moves Beyond Prompts to Platforms, Harnesses, and Evals
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/07/production-ai-platforms-evals/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.976 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, harness, eval.

## 4. OpenWiki 0.2 brings OKF to codebase documentation
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/openwiki-0-2-adds-okf-support
- Score: 2.775 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 5. Cloud Native Infrastructure Emerges as the Foundation for Trustworthy Agentic AI
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/07/cncf-trustworthy-agentic-ai/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.729 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 6. LQCDMaster: Agentic Scientific Computing for Lattice Quantum Chromodynamics Research
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2607.15001v1
- Score: 2.593 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, eval.

## 7. How Cursor knew Claude Fable 5 was ready for the hardest 1% of problems | Claude by Anthropic
- Type: news | Source: claude_blog
- URL: https://claude.com/blog/working-at-the-frontier-cursor
- Score: 2.55 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 8. Quoting Kimi K3
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/17/kimi-k3/#atom-everything
- Score: 2.514 | Reliability: 1.0
- Why it matters: 

## 9. A scorecard for the AI age
- Type: news | Source: openai_blog
- URL: https://openai.com/index/a-scorecard-for-the-ai-age
- Score: 2.478 | Reliability: 1.0
- Why it matters: 

## 10. Why teens deserve access to safe AI
- Type: news | Source: openai_blog
- URL: https://openai.com/index/why-teens-deserve-access-safe-ai
- Score: 1.955 | Reliability: 1.0
- Why it matters: 

## 11. Digital Pantheon: Simulating and Auditing Coalition Formation with LLM Agents
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2607.15095v1
- Score: 2.434 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 12. codex rust-v0.145.0-alpha.22
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.145.0-alpha.22
- Score: 2.424 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 13. [AINews] Kimi K3 2.8T-A50B: the largest open model ever released; Opus 4.8-class at Sonnet 5 pricing
- Type: release | Source: latent_space
- URL: https://www.latent.space/p/ainews-kimi-k3-28t-a50b-the-largest
- Score: 2.392 | Reliability: 1.0
- Why it matters: 

## 14. Building Managed Agents That Use GitHub Without Exposing Your Token
- Type: news | Source: philschmid
- URL: https://www.philschmid.de/managed-agents-gh
- Score: 2.372 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 15. New in Fleet: Deploy AI agents to Slack in one click
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/new-in-langsmith-fleet-bring-agents-into-slack-in-one-click
- Score: 2.363 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 16. Moonshot AI’s Latest Model Dings Tech Stocks. We’ve Seen This Story Before. - Barron's
- Type: news | Source: search_cn_open_weight_labs
- URL: https://news.google.com/rss/articles/CBMifkFVX3lxTE03MWJjVUE0R3dTODlnLVlTSzl2RFFGWHFEU2RETjgxazlHUHBob19vUEdaWW5lZmI0Q0RiNUU0ZTk5c3h3cHlYTkR3T1AyOXc3ekpLYklkRTZLbEVoWGZGQXNLUU9jQzJKcnpzQlc2cElvY0YwUkpySk5GcnFudw?oc=5
- Score: 2.362 | Reliability: 1.0
- Why it matters: 

## 17. OmniaBench: Benchmarking General AI Agents Across Diverse Scenarios
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2607.14989v1
- Score: 2.329 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 18. claude-code v2.1.212
- Type: release | Source: claude_code_releases
- URL: https://github.com/anthropics/claude-code/releases/tag/v2.1.212
- Score: 2.279 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 19. DriftWorld: Fast World Modeling through Drifting
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2607.15065v1
- Score: 2.193 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 20. Fine-tune video and image models at scale with NVIDIA NeMo Automodel and 🤗 Diffusers
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/nvidia/scale-diffusers-finetuning-nemo-automodel
- Score: 2.167 | Reliability: 1.0
- Why it matters: 

## 21. claude-agent-sdk-python v0.2.121
- Type: release | Source: claude_agent_sdk_python_releases
- URL: https://github.com/anthropics/claude-agent-sdk-python/releases/tag/v0.2.121
- Score: 2.164 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 22. NVIDIA Vera Rubin Maximizes Intelligence per Dollar for Post-Training Workloads — a Key Metric for Agentic AI
- Type: news | Source: nvidia_blog
- URL: https://blogs.nvidia.com/blog/nvidia-vera-rubin-post-training-intelligence-per-dollar/
- Score: 1.866 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

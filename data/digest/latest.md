# Daily AI SOTA Digest - 2026-07-14

Focus: AI Platform Engineering

## 1. Claude at scale on Google Cloud: Frontier AI, built for enterprise production
- Type: news | Source: google_cloud_blog
- URL: https://cloud.google.com/blog/products/ai-machine-learning/claude-at-scale-on-google-cloud-frontier-ai-built-for-enterprise-production/
- Score: 3.069 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 2. How to Debug Coding Agents with LangSmith Traces
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/your-coding-agents-are-a-black-box-heres-how-to-crack-them-open
- Score: 2.896 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, codex, claude code.

## 3. Google and Industry Partners Announce Agentic Resource Discovery Specification for AI Agents
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/07/agentic-resource-discovery-spec/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.696 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 4. Meta's Noninvasive Brain–Computer Interface Brain2Qwerty Achieves 61% Accuracy
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/07/meta-brain-interface/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.682 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 5. Quoting Armin Ronacher
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/14/armin-ronacher/#atom-everything
- Score: 2.68 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 6. Introducing Claude for Teachers
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/claude-for-teachers
- Score: 2.598 | Reliability: 1.0
- Why it matters: 

## 7. How to manage AI investments in the agentic era
- Type: news | Source: openai_blog
- URL: https://openai.com/index/managing-ai-investments-in-agentic-era
- Score: 2.585 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 8. UMoE:Unlocking Every Expert in Domain-Specific Training
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2607.11444v1
- Score: 2.551 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation.

## 9. Heuristic Learning for Active Flow Control Using Coding Agents
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2607.11565v1
- Score: 2.542 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 10. Anthropic commits $10 million to Canadian AI research
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/canadian-ai-research
- Score: 2.399 | Reliability: 1.0
- Why it matters: 

## 11. MAGIC: Transition-Aware Generation of Navigable Multi-Scene Game Worlds with Large Language Models
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2607.11594v1
- Score: 2.42 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 12. How Canada uses Claude
- Type: research | Source: anthropic_research
- URL: https://www.anthropic.com/research/how-canada-uses-claude
- Score: 2.409 | Reliability: 1.0
- Why it matters: 

## 13. lobste.rs is now running on SQLite
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/14/lobsters-sqlite/#atom-everything
- Score: 2.393 | Reliability: 1.0
- Why it matters: 

## 14. Oracle AI agent builder brings no-code, low-code and pro-code together - TechTarget
- Type: news | Source: search_agent_engineering_news
- URL: https://news.google.?oc=5
- Score: 2.376 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 15. codex 0.145.0-alpha.11
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.145.0-alpha.11
- Score: 2.329 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 16. [AINews] Codex usage up >10x in 6 months to 7M users, +1M in the past ~day; did Codex overtake Claude Code??
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/ainews-codex-usage-up-10x-in-6-months
- Score: 2.327 | Reliability: 1.0
- Why it matters: Matches feed focus: codex, claude code.

## 17. claude-code v2.1.208
- Type: release | Source: claude_code_releases
- URL: https://github.com/anthropics/claude-code/releases/tag/v2.1.208
- Score: 2.226 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, claude code.

## 18. How sales teams use ChatGPT Work
- Type: news | Source: openai_blog
- URL: https://openai.com/academy/codex-for-work/how-sales-teams-use-codex
- Score: 2.196 | Reliability: 1.0
- Why it matters: 

## 19. ResearchQA: Benchmarking Citation-Grounded Question-Answering on Scientific Papers
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2607.11074v1
- Score: 2.136 | Reliability: 1.0
- Why it matters: Matches feed focus: harness, evaluation.

## 20. claude-agent-sdk-python v0.2.118
- Type: release | Source: claude_agent_sdk_python_releases
- URL: https://github.com/anthropics/claude-agent-sdk-python/releases/tag/v0.2.118
- Score: 2.125 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 21. How data science teams use ChatGPT Work
- Type: news | Source: openai_blog
- URL: https://openai.com/academy/codex-for-work/how-data-science-teams-use-codex
- Score: 2.044 | Reliability: 1.0
- Why it matters: 

## 22. Accelerating software delivery with agentic QA automation using Amazon Nova Act – Part 2
- Type: news | Source: aws_ml_blog
- URL: https://aws.amazon.com/blogs/machine-learning/accelerating-software-delivery-with-agentic-qa-automation-using-amazon-nova-act-part-2/
- Score: 2.0 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, software delivery.

## 23. Empowering India’s next generation of innovators with ATL Saathi
- Type: news | Source: google_deepmind_blog
- URL: https://deepmind.google/blog/empowering-indias-next-generation-of-innovators-with-atl-saathi/
- Score: 1.985 | Reliability: 1.0
- Why it matters: 

## 24. vllm v0.25.1
- Type: release | Source: vllm_releases
- URL: https://github.com/vllm-project/vllm/releases/tag/v0.25.1
- Score: 1.962 | Reliability: 1.0
- Why it matters:

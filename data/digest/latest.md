# Daily AI SOTA Digest - 2026-07-02

Focus: AI Platform Engineering

## 1. OpenWiki: Open Source Repo Documentation for Coding Agents
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/introducing-openwiki-an-open-source-agent-for-repo-documentation
- Score: 3.671 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 2. Show HN: Enola-A deterministic architecture graph for developers and AI agents
- Type: news | Source: hackernews_ai
- URL: https://github.com/enola-labs/enola/tree/main
- Score: 3.183 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 3. How to Use RLMs in Deep Agents
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/how-to-use-rlms-in-deep-agents
- Score: 3.115 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 4. QuasiMoTTo: Quasi-Monte Carlo Test-Time Scaling
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2607.01179v1
- Score: 2.903 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 5. Are Performance-Optimization Benchmarks Reliably Measuring Coding Agents?
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2607.01211v1
- Score: 2.822 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 6. MemSyco-Bench: Benchmarking Sycophancy in Agent Memory
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2607.01071v1
- Score: 2.759 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 7. Apple Extends Private Cloud Compute to Google Cloud for the First Time
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/07/apple-pcc-google-cloud/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.659 | Reliability: 1.0
- Why it matters: 

## 8. Presentation: Graph RAG: Building Smarter Retrieval Workflows with Knowledge Graphs
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/presentations/graph-rag-llm/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.658 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 9. Skill engineering and the case against one-shot AI design
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/skill-engineering-design
- Score: 2.492 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 10. Autoresearch: The feedback loop behind self-improving agents
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/autoresearch-introspection
- Score: 2.467 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 11. ScarfBench: Benchmarking AI Agents for Enterprise Java Framework Migration
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/ibm-research/scarfbench
- Score: 2.719 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 12. TiRex-2: Generalizing TiRex to Multivariate Data and Streaming
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2607.01204v1
- Score: 2.688 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 13. codex 0.143.0-alpha.33
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.143.0-alpha.33
- Score: 2.327 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 14. claude-code v2.1.198
- Type: release | Source: claude_code_releases
- URL: https://github.com/anthropics/claude-code/releases/tag/v2.1.198
- Score: 2.126 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 15. NVIDIA Unlocks AI Compute at Scale, Inviting Partners to Power the AI Infrastructure Buildout
- Type: news | Source: nvidia_blog
- URL: https://blogs.nvidia.com/blog/nvidia-unlocks-ai-compute-at-scale-capital-partners-to-power-ai-infrastructure-buildout/
- Score: 1.567 | Reliability: 1.0
- Why it matters:

# Daily AI SOTA Digest - 2026-08-08

Focus: AI Platform Engineering

## 1. Now we have a timeline of the OpenAI accidental attack against Hugging Face
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Aug/7/openai-timeline/#atom-everything
- Score: 2.975 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 2. Managed Deep Agents is now in public beta
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/managed-deep-agents-is-now-in-public-beta
- Score: 2.756 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 3. Obscura: Headless browser engine. For web scraping and AI agent automation
- Type: news | Source: hackernews_ai
- URL: https://github.com/h4ckf0r0day/obscura
- Score: 2.747 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 4. Cloudflare Launches Persistent, Stateful, Computer-like Environments for Agents
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/08/cloudflare-computer-agents/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.715 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 5. Moonlight & Mayhem (Raccoon Heist by Codex + GPT-5.6 Sol Ultra)
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Aug/7/moonlight-mayhem/#atom-everything
- Score: 2.693 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, codex, claude code.

## 6. [AINews] Zawinski's Law of MultiAgents
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/ainews-zawinskis-law-of-multiagents
- Score: 2.662 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 7. Presentation: Rewriting All of Spotify's Code Base, All the Time
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/presentations/spotify-ai-codebase-migration-agent/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.649 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 8. Responding to the next frontier of critical cyber capabilities
- Type: news | Source: openai_blog
- URL: https://openai.com/index/responding-next-frontier-critical-cyber-capabilities
- Score: 2.404 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 9. How HSP GRUPPE builds AI capabilities for tax advisory
- Type: news | Source: openai_blog
- URL: https://openai.com/index/hsp-gruppe
- Score: 2.11 | Reliability: 1.0
- Why it matters: 

## 10. Improving Fable 5 Safeguards
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/improving-fable-5-s-biology-safeguards
- Score: 2.089 | Reliability: 1.0
- Why it matters: 

## 11. LangSmith LLM Gateway: runtime governance built into the agent lifecycle
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/introducing-llm-gateway
- Score: 2.623 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 12. HarnessOpt-Bench: Evaluating LLMs at Harness Optimization
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2608.06301v1
- Score: 2.473 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, harness, evaluation.

## 13. claude-agent-sdk-python v0.2.134
- Type: release | Source: claude_agent_sdk_python_releases
- URL: https://github.com/anthropics/claude-agent-sdk-python/releases/tag/v0.2.134
- Score: 2.414 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 14. codex 0.148.0-alpha.5
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.148.0-alpha.5
- Score: 2.401 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 15. claude-code v2.1.225
- Type: release | Source: claude_code_releases
- URL: https://github.com/anthropics/claude-code/releases/tag/v2.1.225
- Score: 2.369 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, claude code.

## 16. langgraph-checkpoint==4.2.0
- Type: release | Source: langgraph_releases
- URL: https://github.com/langchain-ai/langgraph/releases/tag/checkpoint%3D%3D4.2.0
- Score: 2.362 | Reliability: 1.0
- Why it matters: 

## 17. Kimi K3 escapes security test sandbox, researchers say - briefs.co
- Type: news | Source: search_cn_open_weight_labs
- URL: https://news.google.com/rss/articles/CBMihwFBVV95cUxQTF9PdzEteVVXdXJCakJCWEw5aGJteUZzSE9VNDd5YzZacVdmTUVpZXlhaFJSLURzYnBTdks0emtDSDRLand6aHZxdDdvR2tMUFZTQWROdVFkdTB6UklYWWNlZlVhZVJ5RGcxYmtwUEJxQ0lvX0s4bW41ZlF5QzZTckhJN09URG8?oc=5
- Score: 2.336 | Reliability: 1.0
- Why it matters: 

## 18. PRISM: Distribution-Gated Flow Matching for Controllable Unpaired Image Translation
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2608.06240v1
- Score: 2.32 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 19. Domain-Grounded Candidate Selection for Agentic Image Editing: A Shadow Removal Case
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2608.06075v1
- Score: 2.237 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, eval.

## 20. Efficient Decode Context Parallelism with vLLM for Long Context Workloads
- Type: news | Source: vllm_blog
- URL: https://vllm.ai/blog/2026-08-07-decode-context-parallelism
- Score: 2.181 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 21. Robot Learning from Human Demonstrations: Handwritten Alphabet Trajectories and Human-Likeness Evaluation
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2608.06221v1
- Score: 2.143 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 22. TutorMoments: Do AI tutors know when to help and when to hold back?
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/allenai/tutormoments
- Score: 2.039 | Reliability: 1.0
- Why it matters: 

## 23. vllm v0.27.0rc1
- Type: release | Source: vllm_releases
- URL: https://github.com/vllm-project/vllm/releases/tag/v0.27.0rc1
- Score: 1.381 | Reliability: 1.0
- Why it matters: 

## 24. Determining playoff clinching scenarios in the NHL using constraint programming
- Type: news | Source: aws_ml_blog
- URL: https://aws.amazon.com/blogs/machine-learning/determining-playoff-clinching-scenarios-in-the-nhl-using-constraint-programming/
- Score: 1.335 | Reliability: 1.0
- Why it matters:

# Daily AI SOTA Digest - 2026-07-17

Focus: AI Platform Engineering

## 1. Kimi K3, and what we can still learn from the pelican benchmark
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/16/kimi-k3/#atom-everything
- Score: 3.138 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation.

## 2. QCon AI Boston: Production AI Moves Beyond Prompts to Platforms, Harnesses, and Evals
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/07/production-ai-platforms-evals/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 3.098 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, harness, eval.

## 3. OpenWiki 0.2 brings OKF to codebase documentation
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/openwiki-0-2-adds-okf-support
- Score: 2.88 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 4. Cloud Native Infrastructure Emerges as the Foundation for Trustworthy Agentic AI
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/07/cncf-trustworthy-agentic-ai/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.843 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 5. Browser automation CLI built for AI agents
- Type: news | Source: hackernews_ai
- URL: https://github.com/browser-act/skills
- Score: 2.829 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 6. LQCDMaster: Agentic Scientific Computing for Lattice Quantum Chromodynamics Research
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2607.15001v1
- Score: 2.655 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, eval.

## 7. [AINews] Kimi K3 2.8T-A50B: the largest open model ever released; Opus 4.8-class at Sonnet 5 pricing
- Type: release | Source: latent_space
- URL: https://www.latent.space/p/ainews-kimi-k3-28t-a50b-the-largest
- Score: 2.492 | Reliability: 1.0
- Why it matters: 

## 8. Digital Pantheon: Simulating and Auditing Coalition Formation with LLM Agents
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2607.15095v1
- Score: 2.49 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 9. New in Fleet: Deploy AI agents to Slack in one click
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/new-in-langsmith-fleet-bring-agents-into-slack-in-one-click
- Score: 2.453 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 10. Firefox in WebAssembly
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/16/firefox-in-webassembly/#atom-everything
- Score: 2.451 | Reliability: 1.0
- Why it matters: 

## 11. China's 2.8-trillion-parameter Kimi K3 beats Claude Fable 5 in Frontend Code Arena benchmark— Moonshot AI delivers largest open-weight AI model ever, as China works around U.S. compute limits - Tom's Hardware
- Type: news | Source: search_cn_open_weight_labs
- URL: https://news.google.?oc=5
- Score: 2.417 | Reliability: 1.0
- Why it matters: 

## 12. OmniaBench: Benchmarking General AI Agents Across Diverse Scenarios
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2607.14989v1
- Score: 2.383 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 13. NVIDIA Nemotron 3 Embed Ranks #1 Overall on RTEB, Advancing Agentic Retrieval
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/nvidia/nemotron-3-embed-wins-rteb
- Score: 2.381 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, eval.

## 14. claude-code v2.1.212
- Type: release | Source: claude_code_releases
- URL: https://github.com/anthropics/claude-code/releases/tag/v2.1.212
- Score: 2.358 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 15. DriftWorld: Fast World Modeling through Drifting
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2607.15065v1
- Score: 2.244 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 16. claude-agent-sdk-python v0.2.121
- Type: release | Source: claude_agent_sdk_python_releases
- URL: https://github.com/anthropics/claude-agent-sdk-python/releases/tag/v0.2.121
- Score: 2.239 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 17. codex 0.145.0-alpha.20
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.145.0-alpha.20
- Score: 2.203 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

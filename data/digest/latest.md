# Daily AI SOTA Digest - 2026-05-17

Focus: AI Platform Engineering

## 1. Databricks brings GPT-5.5 to enterprise agent workflows
- Type: news | Source: openai_blog
- URL: https://openai.com/index/databricks
- Score: 3.162 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 2. OpenAI Open-Sources Symphony, a SPEC.md for Autonomous Coding Agent Orchestration
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/05/openai-symphony-agents/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 3.093 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 3. ATLAS: Agentic or Latent Visual Reasoning? One Word is Enough for Both
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2605.15198v1
- Score: 3.088 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 4. Show HN: Multi-agent orchestration layer for experimentation
- Type: news | Source: hackernews_ai
- URL: https://github.com/theo-kirby/chassis
- Score: 2.88 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 5. datasette-ip-rate-limit 0.1a0
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/May/14/datasette-ip-rate-limit/#atom-everything
- Score: 2.753 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 6. How business operations teams use Codex
- Type: news | Source: openai_blog
- URL: https://openai.com/academy/codex-for-work/how-business-operations-teams-use-codex
- Score: 2.682 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 7. How sales teams use Codex
- Type: news | Source: openai_blog
- URL: https://openai.com/academy/codex-for-work/how-sales-teams-use-codex
- Score: 2.682 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 8. Sea's View on the Future of Agentic Software Development with Codex
- Type: news | Source: openai_blog
- URL: https://openai.com/index/sea-david-chen
- Score: 2.679 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 9. Anthropic Introduces Routines for Claude Code Automation
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/05/anthropic-routines-claude/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.649 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 10. Claude For Small Business
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/claude-for-small-business
- Score: 2.415 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 11. Warelay -> OpenClaw
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/May/16/openclaw-names/#atom-everything
- Score: 2.647 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 12. v0.2.82
- Type: release | Source: claude_agent_sdk_python_releases
- URL: https://github.com/anthropics/claude-agent-sdk-python/releases/tag/v0.2.82
- Score: 2.496 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 13. v2.1.142
- Type: release | Source: claude_code_releases
- URL: https://github.com/anthropics/claude-code/releases/tag/v2.1.142
- Score: 2.489 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 14. 2028 Ai Leadership
- Type: research | Source: anthropic_research
- URL: https://www.anthropic.com/research/2028-ai-leadership
- Score: 2.448 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 15. Best Practices For Computer And Browser Use With Claude
- Type: news | Source: claude_blog
- URL: https://claude.com/blog/best-practices-for-computer-and-browser-use-with-claude
- Score: 2.425 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 16. [AINews] Cerebras' $60B IPO: Slowly, then All at Once
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/ainews-cerebras-60b-ipo-slowly-then
- Score: 2.383 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 17. Granite Embedding Multilingual R2: Open Apache 2.0 Multilingual Embeddings with 32K Context — Best Sub-100M Retrieval Quality
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/ibm-granite/granite-embedding-multilingual-r2
- Score: 2.249 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 18. Forgetting That Sticks: Quantization-Permanent Unlearning via Circuit Attribution
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2605.15138v1
- Score: 2.238 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 19. Pwc Expanded Partnership
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/pwc-expanded-partnership
- Score: 2.235 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

## 20. 0.131.0-alpha.22
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.131.0-alpha.22
- Score: 2.092 | Reliability: 1.0
- Why it matters: Potential relevance to AI platform engineering; verify practical impact.

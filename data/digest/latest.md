# Daily AI SOTA Digest - 2026-07-31

Focus: AI Platform Engineering

## 1. Advancing the price-performance frontier with GPT‑5.6
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/30/luna-price-drop/#atom-everything
- Score: 3.479 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, codex.

## 2. Investigating three real-world incidents in our cybersecurity evaluations
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/30/three-real-world-incidents/#atom-everything
- Score: 3.267 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 3. Frontis-MA1: Training an AI4AI Model towards Recursive Self-Improvement in Machine Learning Engineering
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2607.28568v1
- Score: 2.989 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation, codex.

## 4. Why Are GUI Agents Correct but Late? Decode on the Decision-Time Critical Path, Tested with Pre-Compiled Policy Trees
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2607.28399v1
- Score: 2.777 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 5. Rethinking Inference-Time Scaling in Local Computer-Use Agents: Failure Modes and Compute Tradeoffs
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2607.28573v1
- Score: 2.757 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, eval.

## 6. Introducing Align Evals: Streamlining LLM Application Evaluation
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/introducing-align-evals
- Score: 2.733 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 7. LangSmith LLM Gateway: runtime governance built into the agent lifecycle
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/introducing-llm-gateway
- Score: 2.649 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 8. Gemini Robotics ER 2: powering robotics with video understanding, task orchestration, and multi-robot collaboration
- Type: news | Source: google_deepmind_blog
- URL: https://deepmind.google/blog/gemini-robotics-er-2-powering-robotics-with-video-understanding-task-orchestration-and-multi-robot-collaboration/
- Score: 2.319 | Reliability: 1.0
- Why it matters: 

## 9. How avatarin built a 24/7 retail agent with GPT-Realtime
- Type: news | Source: openai_blog
- URL: https://openai.com/index/avatarin
- Score: 2.075 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 10. We’re launching Lyria 3.5 in Google Flow Music, with advances across musicality, lyrics, vocals, and creative control
- Type: news | Source: google_deepmind_blog
- URL: https://deepmind.google/blog/were-launching-lyria-35-in-google-flow-music-with-advances-across-musicality-lyrics-vocals-and-creative-control/
- Score: 1.906 | Reliability: 1.0
- Why it matters: 

## 11. Qwen-UI-Agent Technical Report: Toward Next-Generation Real-World Centric Foundation GUI Agents
- Type: paper | Source: arxiv_cn_open_weight_reports
- URL: http://arxiv.org/abs/2607.28227v1
- Score: 2.659 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, harness, evaluation.

## 12. ORCA-bench: How Ready Are Language Model Agents for Oncall?
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2607.28545v1
- Score: 2.619 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 13. Do more with less: How GKE can reduce your cost per agent by 75%
- Type: news | Source: google_cloud_blog
- URL: https://cloud.google.com/blog/products/containers-kubernetes/reduce-your-agents-costs-with-gke-agent-sandbox/
- Score: 2.44 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 14. Why is Z.AI stock surging today? - Investing.com
- Type: news | Source: search_cn_open_weight_labs
- URL: https://news.google.com/rss/articles/CBMilwFBVV95cUxQV1RoRUtkMk1kdXpHUkU0XzVJaDRmejJjYndIZWNCdkNtUEVtOEh3Vlk1U01XWEhLYUtBbXl0VW1LQXhxcnRCbWlfZ0RQMjEyNjhnSUtCdEVvNUlPd1FRR0I1S0xYODZOYzdXN2dBeVN3c3hiYkwyeUNzTW5aZXpQbk9wdTBobXh0aV9kc1Q4QS15UUw5Wlh3?oc=5
- Score: 2.391 | Reliability: 1.0
- Why it matters: 

## 15. langgraph-checkpoint-sqlite==3.1.1
- Type: release | Source: langgraph_releases
- URL: https://github.com/langchain-ai/langgraph/releases/tag/checkpointsqlite%3D%3D3.1.1
- Score: 2.336 | Reliability: 1.0
- Why it matters: 

## 16. Ontologies Are So Back: Why AI Agents Are Reviving the Semantic Web
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/ontologies-agentic-systems
- Score: 2.271 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 17. Stacked sessions and pull requests in the GitHub Copilot app
- Type: news | Source: github_blog_ai_ml
- URL: https://github.blog/ai-and-ml/github-copilot/stacked-sessions-and-pull-requests-in-the-github-copilot-app/
- Score: 2.159 | Reliability: 1.0
- Why it matters: 

## 18. Release 2.71.0 corresponding to NGC container 26.07
- Type: release | Source: triton_releases
- URL: https://github.com/triton-inference-server/server/releases/tag/v2.71.0
- Score: 2.092 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 19. [AINews] AI is eating Finance; AIE NYC now open
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/ainews-ai-is-eating-finance-aie-nyc
- Score: 2.064 | Reliability: 1.0
- Why it matters: 

## 20. How enabling two settings tripled our scores on the ARC-AGI-3 benchmark
- Type: news | Source: openai_blog
- URL: https://openai.com/index/how-two-settings-tripled-our-arc-agi-3-scores
- Score: 1.811 | Reliability: 1.0
- Why it matters: 

## 21. Deploying Kimi K3 on AWS
- Type: news | Source: aws_ml_blog
- URL: https://aws.amazon.com/blogs/machine-learning/deploying-kimi-k3-on-aws/
- Score: 1.365 | Reliability: 1.0
- Why it matters:

# Daily AI SOTA Digest - 2026-06-29

Focus: AI Platform Engineering

## 1. vllm v0.24.0
- Type: release | Source: vllm_releases
- URL: https://github.com/vllm-project/vllm/releases/tag/v0.24.0
- Score: 3.438 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 2. Ornith-1.0: Self-Scaffolding LLMs for Agentic Coding
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jun/29/ornith/#atom-everything
- Score: 3.324 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, harness.

## 3. “It’s Hard to Eval” Is a Product Smell
- Type: news | Source: hamel_husain
- URL: https://hamel.dev/blog/posts/eval-smell/
- Score: 3.298 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 4. Micro-Agent: Beat Frontier Models with Collaboration inside Model API
- Type: news | Source: vllm_blog
- URL: https://vllm.ai/blog/2026-06-29-micro-agent-frontier-models
- Score: 2.998 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 5. Cloud CISO Perspectives: How Google Cloud Security uses AI internally
- Type: news | Source: google_cloud_blog
- URL: https://cloud.google.com/blog/products/identity-security/cloud-ciso-perspectives-how-google-cloud-security-uses-ai-internally/
- Score: 2.964 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, harness, evaluation.

## 6. HAT-4D: Lifting Monocular Video for 4D Multi-Object Interactions via Human-Agent Collaboration
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2606.28215v1
- Score: 2.882 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation.

## 7. Mapping Europe’s AI Workforce Opportunity
- Type: news | Source: openai_blog
- URL: https://openai.com/index/mapping-ai-jobs-transition-eu
- Score: 2.844 | Reliability: 1.0
- Why it matters: 

## 8. Inside Target’s LLM-Based System for Semantic Matching in Marketing Forecast Pipelines
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/06/target-ai-campaign-forecasting/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.83 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 9. AI Tools Accelerates Coding, but Not Overall Software Delivery, GitLab Research Finds
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/06/ai-coding-outpaces-governance/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.819 | Reliability: 1.0
- Why it matters: Matches feed focus: software delivery.

## 10. HP Inc. launches Frontier strategic partnership with OpenAI
- Type: news | Source: openai_blog
- URL: https://openai.com/index/hp-frontier-partnership
- Score: 2.457 | Reliability: 1.0
- Why it matters: 

## 11. Towards Automating Scientific Review with Google's Paper Assistant Tool
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2606.28277v1
- Score: 2.806 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation.

## 12. From Detection to Action: Using LLM Agents for Fault-Tolerant Control
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2606.28011v1
- Score: 2.72 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, eval.

## 13. Using Local Coding Agents
- Type: news | Source: sebastian_raschka
- URL: https://magazine.sebastianraschka.com/p/using-local-coding-agents
- Score: 2.596 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, harness, codex.

## 14. Open-source AI agent workflow for auditing Solidity smart contracts
- Type: news | Source: hackernews_ai
- URL: https://github.com/chain-shield/ai-agent-audit
- Score: 2.533 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 15. Quoting Jon Udell
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jun/28/jon-udell/#atom-everything
- Score: 2.531 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 16. An Empirical Analysis of Factual Errors in Human-Written Text and its Application
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2606.27959v1
- Score: 2.455 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 17. Latest open artifacts (#22): Zyphra, Cohere, and Poolside are expanding the breadth of the ecosystem
- Type: news | Source: interconnects
- URL: https://www.interconnects.ai/p/artifacts-22-zyphra-cohere-and-poolside
- Score: 2.448 | Reliability: 1.0
- Why it matters: 

## 18. codex 0.142.4
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.142.4
- Score: 2.419 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 19. DiScoFormer: One transformer for density and score, across distributions
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/allenai/discoformer
- Score: 2.174 | Reliability: 1.0
- Why it matters: 

## 20. Claude Meets Blackwell Ultra: Anthropic’s Models Now Run on NVIDIA GB300 in Azure
- Type: news | Source: nvidia_blog
- URL: https://blogs.nvidia.com/blog/anthropic-nvidia-gb300-blackwell-ultra-microsoft-azure/
- Score: 2.015 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

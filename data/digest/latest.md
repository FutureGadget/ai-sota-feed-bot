# Daily AI SOTA Digest - 2026-07-14

Focus: AI Platform Engineering

## 1. Securing the AI supply chain on GKE: Introducing k8s-aibom for automated AI BOMs
- Type: news | Source: google_cloud_blog
- URL: https://cloud.google.com/blog/products/identity-security/introducing-k8s-aibom-on-gke-for-automated-ai-bills-of-materials/
- Score: 2.542 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, harness, evaluation.

## 2. UMoE:Unlocking Every Expert in Domain-Specific Training
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2607.11444v1
- Score: 2.725 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation.

## 3. Heuristic Learning for Active Flow Control Using Coding Agents
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2607.11565v1
- Score: 2.719 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 4. MAGIC: Transition-Aware Generation of Navigable Multi-Scene Game Worlds with Large Language Models
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2607.11594v1
- Score: 2.588 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 5. [AINews] Codex usage up >10x in 6 months to 7M users, +1M in the past ~day; did Codex overtake Claude Code??
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/ainews-codex-usage-up-10x-in-6-months
- Score: 2.532 | Reliability: 1.0
- Why it matters: Matches feed focus: codex, claude code.

## 6. claude-code v2.1.208
- Type: release | Source: claude_code_releases
- URL: https://github.com/anthropics/claude-code/releases/tag/v2.1.208
- Score: 2.52 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, claude code.

## 7. Using uvx in GitHub Actions in a cache-friendly way
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/14/uvx-github-actions-cache/#atom-everything
- Score: 2.506 | Reliability: 1.0
- Why it matters: 

## 8. codex 0.144.4
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.144.4
- Score: 2.49 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 9. Empowering India’s next generation of innovators with ATL Saathi
- Type: news | Source: google_deepmind_blog
- URL: https://deepmind.google/blog/empowering-indias-next-generation-of-innovators-with-atl-saathi/
- Score: 2.164 | Reliability: 1.0
- Why it matters: 

## 10. Working at the frontier: How Hebbia builds AI for financial diligence that can't miss a detail | Claude by Anthropic
- Type: news | Source: claude_blog
- URL: https://claude.com/blog/working-at-the-frontier-how-hebbia-builds-ai-for-financial-diligence-that-cant-miss-a-detail
- Score: 2.105 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 11. DOOMQL
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/13/doomql/#atom-everything
- Score: 2.458 | Reliability: 1.0
- Why it matters: 

## 12. How DoorDash Built an AI Shopping Assistant That Doesn’t Rely on the LLM Alone
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/07/doordash-ai-ask-assistant/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.436 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 13. claude-agent-sdk-python v0.2.118
- Type: release | Source: claude_agent_sdk_python_releases
- URL: https://github.com/anthropics/claude-agent-sdk-python/releases/tag/v0.2.118
- Score: 2.421 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 14. How Claude's values vary by model and language
- Type: research | Source: anthropic_research
- URL: https://www.anthropic.com/research/claude-values-models-languages
- Score: 2.334 | Reliability: 1.0
- Why it matters: 

## 15. ResearchQA: Benchmarking Citation-Grounded Question-Answering on Scientific Papers
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2607.11074v1
- Score: 2.329 | Reliability: 1.0
- Why it matters: Matches feed focus: harness, evaluation.

## 16. The Path to Sovereign Data: Challenges and Priorities in Local-First Computing
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/07/data-ownership-localfirst/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.269 | Reliability: 1.0
- Why it matters: 

## 17. 6 months to live for open models
- Type: news | Source: interconnects
- URL: https://www.interconnects.ai/p/6-months-to-live-for-open-models
- Score: 1.866 | Reliability: 1.0
- Why it matters: 

## 18. Launching UI for generative AI inference recommendations in Amazon SageMaker AI
- Type: news | Source: aws_ml_blog
- URL: https://aws.amazon.com/blogs/machine-learning/launching-ui-for-generative-ai-inference-recommendations-in-amazon-sagemaker-ai/
- Score: 1.537 | Reliability: 1.0
- Why it matters: 

## 19. v0.25.1: [Bugfix] Guard mixed-dtype allreduce RMSNorm quant fusions (#48330)
- Type: release | Source: vllm_releases
- URL: https://github.com/vllm-project/vllm/releases/tag/v0.25.1
- Score: 1.299 | Reliability: 1.0
- Why it matters:

# Daily AI SOTA Digest - 2026-06-15

Focus: AI Platform Engineering

## 1. Why AI hasn’t replaced software engineers, and won’t
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jun/14/why-ai-hasnt-replaced-software-engineers/#atom-everything
- Score: 3.175 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 2. Decoupled Mixture-of-Experts for Parametric Knowledge Injection
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2606.14243v1
- Score: 3.068 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 3. When to Write and When to Suppress: Route-Specialized Dual Adapters for Memory-Assisted Knowledge Editing
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2606.14668v1
- Score: 3.064 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 4. From Shield to Target: Denial-of-Service Attacks on LLM-Based Agent Guardrails
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2606.14517v1
- Score: 3.001 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 5. vllm v0.23.0
- Type: release | Source: vllm_releases
- URL: https://github.com/vllm-project/vllm/releases/tag/v0.23.0
- Score: 2.942 | Reliability: 1.0
- Why it matters: 

## 6. Introducing the OpenAI Partner Network
- Type: news | Source: openai_blog
- URL: https://openai.com/index/introducing-openai-partner-network
- Score: 2.769 | Reliability: 1.0
- Why it matters: 

## 7. Coding Agent Sandboxes Don't Solve Credential Authorization
- Type: news | Source: hackernews_ai
- URL: https://www.permit.io/blog/coding-agent-sandboxes-credentials
- Score: 2.649 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 8. Anthropic Releases and Temporarily Suspends Claude Fable 5
- Type: release | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/06/claude-5-release/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.641 | Reliability: 1.0
- Why it matters: 

## 9. Claude Fable 5 and Claude Mythos 5
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/claude-fable-5-mythos-5
- Score: 2.496 | Reliability: 1.0
- Why it matters: 

## 10. Results from first Anthropic Public Record
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/anthropic-public-record
- Score: 2.494 | Reliability: 1.0
- Why it matters: 

## 11. Capability Minimization as a Safety Primitive: Risk-Aware Causal Gating for Least-Privilege LLM Agents
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2606.13884v1
- Score: 2.944 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 12. AWS Introduces Durable Storage Option for ElastiCache for Valkey
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/06/elasticache-valkey-durability/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.54 | Reliability: 1.0
- Why it matters: 

## 13. Quoting Julia Evans
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jun/15/julia-evans/#atom-everything
- Score: 2.471 | Reliability: 1.0
- Why it matters: 

## 14. olmo-eval: An evaluation workbench for the model development loop
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/allenai/olmo-eval
- Score: 2.335 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 15. TCS and Anthropic partner to bring Claude to regulated industries
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/tcs-anthropic-partnership
- Score: 2.29 | Reliability: 1.0
- Why it matters: 

## 16. codex 0.140.0-alpha.19
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.140.0-alpha.19
- Score: 2.206 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

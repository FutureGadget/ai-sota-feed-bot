# Daily AI SOTA Digest - 2026-09-19

Focus: AI Platform Engineering

## 1. AI Evals: Everything You Need to Know
- Type: news | Source: hamel_husain
- URL: https://hamel.dev/blog/posts/evals-faq/
- Score: 3.758 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation, codex.

## 2. DoorDash Uses Multi Agent LLMs to Clean up 60,000 Feature Flags
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/09/doordash-feature-flag-cleanup/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.855 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 3. WSO2 Releases Agent Manager as Enterprises Look to Control Growing AI Agent Sprawl
- Type: release | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/09/ws02-agent-manager/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.705 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 4. Quoting Thariq Shihipar
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Sep/18/thariq-shihipar/
- Score: 2.659 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, harness, claude code.

## 5. Gemini Hacked Three Companies in First Known Breakout by Google’s AI
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Sep/18/gemini-hacked-three-companies/
- Score: 2.616 | Reliability: 1.0
- Why it matters: 

## 6. What Is Jev? A Guide to TypeSafe AI’s System One Model
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/building-a-harness-with-jev
- Score: 2.54 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 7. codex 0.155.1
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.155.1
- Score: 2.456 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 8. Partnering with Accenture on embedded evaluation
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/accenture-embedded-evaluation
- Score: 2.107 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 9. How Cooley is accelerating IPO work with ChatGPT
- Type: news | Source: openai_blog
- URL: https://openai.com/index/cooley-gopublic
- Score: 1.76 | Reliability: 1.0
- Why it matters: 

## 10. Introducing Astra for Law
- Type: news | Source: openai_blog
- URL: https://openai.com/index/astra-for-law
- Score: 1.712 | Reliability: 1.0
- Why it matters: 

## 11. Chronicle: Cut-Point Replay for Regression Testing of LLM Agents
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2609.20625v1
- Score: 2.359 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 12. Changing the game: Using agentic AI to secure infrastructure code
- Type: news | Source: google_cloud_blog
- URL: https://cloud.google.com/blog/topics/systems/using-ai-agents-to-secure-google-infrastructure/
- Score: 2.332 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, harness, eval.

## 13. claude-code v2.1.277
- Type: release | Source: claude_code_releases
- URL: https://github.com/anthropics/claude-code/releases/tag/v2.1.277
- Score: 2.315 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, claude code.

## 14. Should you read the code, is RAG dead, and did Skills kill MCP?
- Type: news | Source: github_blog_ai_ml
- URL: https://github.blog/ai-and-ml/should-you-read-the-code-is-rag-dead-and-did-skills-kill-mcp/
- Score: 2.124 | Reliability: 1.0
- Why it matters: 

## 15. Alibaba Qwen Releases Qwen3.8-Omni-Flash: A 1M-Context Omni-Modal Model Built Around Agentic Audio-Video Understanding and Tool Use
- Type: release | Source: search_cn_open_weight_labs
- URL: https://news.google.?oc=5
- Score: 2.05 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 16. How Included Health Built Federated Healthcare Agents with LangGraph and Deep Agents
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/how-included-health-built-federated-agents-for-healthcare-navigation-with-deep-agents-and-langgraph
- Score: 1.995 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 17. Deploy Hugging Face models on Amazon SageMaker AI with coding agents
- Type: news | Source: aws_ml_blog
- URL: https://aws.amazon.com/blogs/machine-learning/deploy-hugging-face-models-on-amazon-sagemaker-ai-with-coding-agents/
- Score: 1.97 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 18. claude-agent-sdk-python v0.2.156
- Type: release | Source: claude_agent_sdk_python_releases
- URL: https://github.com/anthropics/claude-agent-sdk-python/releases/tag/v0.2.156
- Score: 1.913 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 19. How Claude is uplifting biomolecular modeling
- Type: research | Source: anthropic_research
- URL: https://www.anthropic.com/research/claude-uplifts-biomolecular-modeling
- Score: 1.909 | Reliability: 1.0
- Why it matters: 

## 20. vllm v0.30.0rc2
- Type: release | Source: vllm_releases
- URL: https://github.com/vllm-project/vllm/releases/tag/v0.30.0rc2
- Score: 1.826 | Reliability: 1.0
- Why it matters: 

## 21. Reimagining advertising with AI
- Type: news | Source: openai_blog
- URL: https://openai.com/index/reimagining-advertising-with-ai
- Score: 1.702 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 22. RADAR: Catch gray failures with anomaly detection
- Type: news | Source: databricks_blog
- URL: https://www.databricks.com/blog/radar-catch-gray-failures-anomaly-detection
- Score: 1.632 | Reliability: 1.0
- Why it matters: 

## 23. Hex turns complex analysis into visual reports with GPT‑6 Astra
- Type: news | Source: openai_blog
- URL: https://openai.com/index/hex-gpt-6-astra
- Score: 1.618 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 24. Your Agent Aced the Task. Will It Do It Again?
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/ibm-research/altk-evolve-consistency
- Score: 1.567 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

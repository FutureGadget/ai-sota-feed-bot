# Daily AI SOTA Digest - 2026-09-22

Focus: AI Platform Engineering

## 1. vllm v0.30.0
- Type: release | Source: vllm_releases
- URL: https://github.com/vllm-project/vllm/releases/tag/v0.30.0
- Score: 3.629 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 2. Jev introduces a new shape of LLM - System One, aka Decision Models
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Sep/21/jev/
- Score: 3.023 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 3. Show HN: Owl24.dev – Hand a production error to your coding agent, get a PR back
- Type: news | Source: hackernews_ai
- URL: https://owl24.dev
- Score: 2.885 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 4. Can Jev Be a Better Agent Evaluator?
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/jev-agent-evals-langsmith
- Score: 2.753 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 5. DolphinBench: Mapping the Pareto Frontier of Agent Memory
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2609.24971v1
- Score: 2.681 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 6. Jev is now available in LangSmith Evals
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/jev-is-now-available-in-langsmith-evals
- Score: 2.472 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 7. [AINews] Xiaomi MiMo-V2.6-Pro 1T-A42B: the new top Open Weights model, trained for $3M
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/ainews-xiaomi-mimo-v26-pro-1t-a42b
- Score: 2.426 | Reliability: 1.0
- Why it matters: 

## 8. Higgsfield AI ships new video features in a day with GPT-6 Astra
- Type: news | Source: openai_blog
- URL: https://openai.com/index/higgsfield-from-prompt-to-production-with-astra
- Score: 2.074 | Reliability: 1.0
- Why it matters: 

## 9. Building standards for the next phase of AI
- Type: news | Source: openai_blog
- URL: https://openai.com/index/building-standards-next-phase-ai
- Score: 2.066 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 10. Partnering with Accenture on embedded evaluation
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/accenture-embedded-evaluation
- Score: 1.417 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 11. llama_index v0.14.25
- Type: release | Source: llamaindex_releases
- URL: https://github.com/run-llama/llama_index/releases/tag/v0.14.25
- Score: 2.366 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 12. codex rust-v0.157.0-alpha.8
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.157.0-alpha.8
- Score: 2.365 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 13. AI Evals: Everything You Need to Know
- Type: news | Source: hamel_husain
- URL: https://hamel.dev/blog/posts/evals-faq/
- Score: 2.325 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation, codex.

## 14. MedRSI: Recursive Self-Improvement for Medical Agents via Clinically Aligned Self-Evolution
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2609.24838v1
- Score: 2.296 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 15. Podcast: Securing AI Agents: Identity, Authorization, and the DPACT Framework
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/podcasts/securing-ai-agents-identity-authorization/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.176 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 16. Presentation: The Agent Harness: Control Planes, Invariants, and Approval Boundaries for Production AI Agents
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/presentations/ai-agent-harness/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.176 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, harness.

## 17. Quoting voxium
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Sep/20/voxium/
- Score: 2.162 | Reliability: 1.0
- Why it matters: Matches feed focus: claude code.

## 18. NVIDIA Isaac ROS 5.0 Advances Agentic, Open Source Robotics Development
- Type: news | Source: nvidia_blog
- URL: https://blogs.nvidia.com/blog/isaac-ros-5-0-agentic-open-source-robotics/
- Score: 2.107 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 19. Transformers now runs llama.cpp quants
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/transformers-llama-cpp-quants
- Score: 2.042 | Reliability: 1.0
- Why it matters: 

## 20. langgraph==1.2.12
- Type: release | Source: langgraph_releases
- URL: https://github.com/langchain-ai/langgraph/releases/tag/1.2.12
- Score: 2.036 | Reliability: 1.0
- Why it matters: 

## 21. Expanding OpenAI Academy with new learning paths
- Type: news | Source: openai_blog
- URL: https://openai.com/index/expanding-openai-academy-with-new-learning-paths
- Score: 1.987 | Reliability: 1.0
- Why it matters: 

## 22. Advisory Group on Mathematics and Artificial Intelligence
- Type: news | Source: openai_blog
- URL: https://openai.com/index/advisory-group-on-mathematics-and-ai
- Score: 1.958 | Reliability: 1.0
- Why it matters: 

## 23. Qwen/Qwen-Image-2.1-PE-I2I released on Hugging Face
- Type: release | Source: hf_qwen_releases
- URL: https://huggingface.co/Qwen/Qwen-Image-2.1-PE-I2I
- Score: 1.546 | Reliability: 1.0
- Why it matters: 

## 24. How Claude is uplifting biomolecular modeling
- Type: research | Source: anthropic_research
- URL: https://www.anthropic.com/research/claude-uplifts-biomolecular-modeling
- Score: 1.467 | Reliability: 1.0
- Why it matters:

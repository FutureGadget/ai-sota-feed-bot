# Daily AI SOTA Digest - 2026-08-19

Focus: AI Platform Engineering

## 1. Launch HN: OneCLI (YC S26) – OSS sandboxed agent harness for teams
- Type: news | Source: hackernews_ai
- URL: https://github.com/onecli/onecli
- Score: 3.089 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, harness, eval.

## 2. Introducing LangSmith Tuned Evaluators
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/introducing-langsmith-tuned-evaluators-starting-with-perceived-error
- Score: 3.045 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 3. DeepSeek Open-Sources the Missing Layer Between AI Models and Agents - HPCwire
- Type: news | Source: search_cn_open_weight_labs
- URL: https://news.google.?oc=5
- Score: 2.646 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 4. EvoTS-Agent: A Self-Evolving LLM Agent for Financial Time Series Change Point Detection
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2608.17933v1
- Score: 2.58 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 5. Whatsapp Tests on Device ML for Scam Detection with Privacy Preserving Analytics
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/08/whatsapp-scam-alert-beta/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.556 | Reliability: 1.0
- Why it matters: 

## 6. AutoResearch: Insight In, Hallucination Out
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2608.17906v1
- Score: 2.534 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 7. Multi-turn Conversational AI from Text to Multimodal Interaction: Data, Models, Evaluation, and Open Challenges
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2608.17605v1
- Score: 2.474 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 8. Replit expands access to software creation with GPT-5.6 Luna
- Type: news | Source: openai_blog
- URL: https://openai.com/index/replit
- Score: 2.301 | Reliability: 1.0
- Why it matters: 

## 9. ChatGPT Ads expands across Europe
- Type: news | Source: openai_blog
- URL: https://openai.com/index/chatgpt-ads-expands-across-europe
- Score: 2.116 | Reliability: 1.0
- Why it matters: 

## 10. Strengthening democratic oversight in national security
- Type: news | Source: openai_blog
- URL: https://openai.com/index/strengthening-democratic-oversight-in-national-security
- Score: 2.062 | Reliability: 1.0
- Why it matters: 

## 11. How Claude is accelerating protein design and analytical chemistry
- Type: research | Source: anthropic_research
- URL: https://www.anthropic.com/research/Claude-accelerates-protein-design
- Score: 2.47 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 12. [AINews] Memory prices up 500% in 12 months
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/ainews-memory-prices-up-500-in-12
- Score: 2.406 | Reliability: 1.0
- Why it matters: 

## 13. Cloudflare WriteGuard Brings Fine-Grained Security Controls for MCP Servers
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/08/cloudflare-writeguard-mcp-safety/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.304 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 14. codex rust-v0.149.0-alpha.2
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.149.0-alpha.2
- Score: 2.235 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 15. AgentCore Payments middleware for LangChain agents
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/langchain-agentcore-payments
- Score: 2.224 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 16. Mojo🔥 is now open source
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Aug/18/mojo-is-now-open-source/
- Score: 2.187 | Reliability: 1.0
- Why it matters: 

## 17. Recirculation
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2608.17981v1
- Score: 2.164 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 18. Designing effective Genie Agents from a single prompt
- Type: news | Source: databricks_blog
- URL: https://www.databricks.com/blog/designing-effective-genie-agents-single-prompt
- Score: 2.086 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 19. Frontier Model Cost and Open-Weights Popularity is Driving Demand for Model Routing
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/glean-model-routing
- Score: 2.066 | Reliability: 1.0
- Why it matters:

# Daily AI SOTA Digest - 2026-06-15

Focus: AI Platform Engineering

## 1. codex 0.140.0
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.140.0
- Score: 3.185 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, codex, claude code.

## 2. Anthropic Explains How Claude Builds Its Own Execution Harnesses
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/06/claude-code-harnesses/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 3.176 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, harness, claude code.

## 3. Why AI hasn’t replaced software engineers, and won’t
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jun/14/why-ai-hasnt-replaced-software-engineers/#atom-everything
- Score: 3.154 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 4. Zero-source LLM Hallucination Detection with Human-like Criteria Probing
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2606.12900v1
- Score: 3.086 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 5. When to Write and When to Suppress: Route-Specialized Dual Adapters for Memory-Assisted Knowledge Editing
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2606.14668v1
- Score: 3.041 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 6. Decoupled Mixture-of-Experts for Parametric Knowledge Injection
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2606.14243v1
- Score: 3.028 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 7. claude-code v2.1.178
- Type: release | Source: claude_code_releases
- URL: https://github.com/anthropics/claude-code/releases/tag/v2.1.178
- Score: 2.936 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval, claude code.

## 8. New OpenAI Academy courses for the next era of work
- Type: news | Source: openai_blog
- URL: https://openai.com/index/academy-courses-applying-ai-at-work
- Score: 2.838 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 9. Introducing the OpenAI Partner Network
- Type: news | Source: openai_blog
- URL: https://openai.com/index/introducing-openai-partner-network
- Score: 2.706 | Reliability: 1.0
- Why it matters: 

## 10. Claude Fable 5 and Claude Mythos 5
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/claude-fable-5-mythos-5
- Score: 2.461 | Reliability: 1.0
- Why it matters: 

## 11. From Shield to Target: Denial-of-Service Attacks on LLM-Based Agent Guardrails
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2606.14517v1
- Score: 2.993 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 12. vllm v0.23.0
- Type: release | Source: vllm_releases
- URL: https://github.com/vllm-project/vllm/releases/tag/v0.23.0
- Score: 2.843 | Reliability: 1.0
- Why it matters: 

## 13. "They screwed us": Personality clashes sent Anthropic's models offline
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jun/15/axios-clashes-anthropics/#atom-everything
- Score: 2.653 | Reliability: 1.0
- Why it matters: 

## 14. Anthropic Releases and Temporarily Suspends Claude Fable 5
- Type: release | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/06/claude-5-release/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.632 | Reliability: 1.0
- Why it matters: 

## 15. claude-agent-sdk-python v0.2.102
- Type: release | Source: claude_agent_sdk_python_releases
- URL: https://github.com/anthropics/claude-agent-sdk-python/releases/tag/v0.2.102
- Score: 2.49 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 16. Results from first Anthropic Public Record
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/anthropic-public-record
- Score: 2.459 | Reliability: 1.0
- Why it matters: 

## 17. Building a 100x Cheaper Trace Judge with Fireworks
- Type: news | Source: langchain_blog
- URL: https://www.langchain.com/blog/building-a-100x-cheaper-trace-judge-with-fireworks
- Score: 2.373 | Reliability: 1.0
- Why it matters: 

## 18. olmo-eval: An evaluation workbench for the model development loop
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/allenai/olmo-eval
- Score: 2.32 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 19. TCS and Anthropic partner to bring Claude to regulated industries
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/tcs-anthropic-partnership
- Score: 2.255 | Reliability: 1.0
- Why it matters: 

## 20. AI Agent Failure Detection and Root Cause Analysis with Strands Evals
- Type: news | Source: aws_ml_blog
- URL: https://aws.amazon.com/blogs/machine-learning/ai-agent-failure-detection-and-root-cause-analysis-with-strands-evals/
- Score: 2.171 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 21. Cloud CISO Perspectives: The 4 lessons that guided AI Threat Defense
- Type: news | Source: google_cloud_blog
- URL: https://cloud.google.com/blog/products/identity-security/cloud-ciso-perspectives-the-4-lessons-that-guided-ai-threat-defense/
- Score: 1.982 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, harness.

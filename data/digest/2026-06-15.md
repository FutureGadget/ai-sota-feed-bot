# Daily AI SOTA Digest - 2026-06-15

Focus: AI Platform Engineering

## 1. Why AI hasn’t replaced software engineers, and won’t
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jun/14/why-ai-hasnt-replaced-software-engineers/#atom-everything
- Score: 3.206 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 2. Decoupled Mixture-of-Experts for Parametric Knowledge Injection
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2606.14243v1
- Score: 3.069 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 3. From Shield to Target: Denial-of-Service Attacks on LLM-Based Agent Guardrails
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2606.14517v1
- Score: 3.002 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 4. Capability Minimization as a Safety Primitive: Risk-Aware Causal Gating for Least-Privilege LLM Agents
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2606.13884v1
- Score: 2.945 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 5. Publishing WASM wheels to PyPI for use with Pyodide
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jun/13/publishing-wasm-wheels/#atom-everything
- Score: 2.93 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 6. vllm v0.23.0
- Type: release | Source: vllm_releases
- URL: https://github.com/vllm-project/vllm/releases/tag/v0.23.0
- Score: 2.839 | Reliability: 1.0
- Why it matters: 

## 7. Introducing the OpenAI Partner Network
- Type: news | Source: openai_blog
- URL: https://openai.com/index/introducing-openai-partner-network
- Score: 2.805 | Reliability: 1.0
- Why it matters: 

## 8. Sync – Quality Control and Project Management System for AI Agents
- Type: news | Source: hackernews_ai
- URL: https://sync.buzz
- Score: 2.64 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 9. Claude Fable 5 and Claude Mythos 5
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/claude-fable-5-mythos-5
- Score: 2.524 | Reliability: 1.0
- Why it matters: 

## 10. Results from first Anthropic Public Record
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/anthropic-public-record
- Score: 2.522 | Reliability: 1.0
- Why it matters: 

## 11. AWS Introduces Durable Storage Option for ElastiCache for Valkey
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/06/elasticache-valkey-durability/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.561 | Reliability: 1.0
- Why it matters: 

## 12. Running the Gauntlet: Re-evaluating the Capabilities of Agents Beyond Familiar Environments
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2606.14397v1
- Score: 2.424 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation.

## 13. olmo-eval: An evaluation workbench for the model development loop
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/allenai/olmo-eval
- Score: 2.337 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 14. TCS and Anthropic partner to bring Claude to regulated industries
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/tcs-anthropic-partnership
- Score: 2.317 | Reliability: 1.0
- Why it matters: 

## 15. codex 0.140.0-alpha.19
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.140.0-alpha.19
- Score: 2.259 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

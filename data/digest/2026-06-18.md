# Daily AI SOTA Digest - 2026-06-18

Focus: AI Platform Engineering

## 1. codex 0.141.0
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.141.0
- Score: 3.144 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, harness, evaluation.

## 2. Are LLMs Ready to Assist Physicians? PhysAssistBench for Interactive Doctor-Patient-EHR Assistance
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2606.18613v1
- Score: 3.034 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation.

## 3. ProductConsistency: Improving Product Identity Preservation in Instruction-Based Image Editing via SFT and RL
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2606.19103v1
- Score: 2.932 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 4. Microsoft Scout, New  Enterprise Autopilot Built on OpenClaw, Announced at Build 2026
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/06/microsoft-scout-openclaw-build/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.885 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 5. A near-autonomous AI chemist improves a challenging reaction in medicinal chemistry
- Type: news | Source: openai_blog
- URL: https://openai.com/index/ai-chemist-improves-reaction
- Score: 2.668 | Reliability: 1.0
- Why it matters: 

## 6. Show HN: Memharness – Bi-temporal memory for AI agents, in one SQLite file
- Type: news | Source: hackernews_ai
- URL: https://github.com/las7/memharness
- Score: 2.644 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, harness.

## 7. Urdu Katib Handwritten Dataset: A Historical Document Dataset for Offline Urdu Handwritten Text Recognition with CRNN-Based Baseline Evaluation
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2606.19139v1
- Score: 2.641 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 8. Anthropic opens Seoul office and announces new partnerships across the Korean AI ecosystem
- Type: news | Source: anthropic_newsroom
- URL: https://www.anthropic.com/news/seoul-office-partnerships-korean-ai-ecosystem
- Score: 2.607 | Reliability: 1.0
- Why it matters: 

## 9. [AINews] Midjourney Medical: scan your organs like you step on a scale
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/ainews-midjourney-medical-scan-your
- Score: 2.56 | Reliability: 1.0
- Why it matters: 

## 10. claude-agent-sdk-python v0.2.104
- Type: release | Source: claude_agent_sdk_python_releases
- URL: https://github.com/anthropics/claude-agent-sdk-python/releases/tag/v0.2.104
- Score: 2.413 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 11. INDEQS: Informed Neural controlled Differential EQuationS
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2606.19138v1
- Score: 2.549 | Reliability: 1.0
- Why it matters: Matches feed focus: eval.

## 12. From the Hugging Face Hub to robot hardware with Strands Agents and LeRobot
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/amazon/strands-lerobot-hub-to-hardware
- Score: 2.439 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 13. claude-code v2.1.181
- Type: release | Source: claude_code_releases
- URL: https://github.com/anthropics/claude-code/releases/tag/v2.1.181
- Score: 2.268 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, claude code.

## 14. France Advances Europe’s AI Future With NVIDIA Technologies
- Type: news | Source: nvidia_blog
- URL: https://blogs.nvidia.com/blog/france-advances-europes-ai-future/
- Score: 2.093 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

# Daily AI SOTA Digest - 2026-07-14

Focus: AI Platform Engineering

## 1. Google's Genkit Ships Agents API with Detached Turns and Human-in-the-Loop for TypeScript and Go
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/07/genkit-agents-api-preview/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.778 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 2. UMoE:Unlocking Every Expert in Domain-Specific Training
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2607.11444v1
- Score: 2.662 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation.

## 3. Heuristic Learning for Active Flow Control Using Coding Agents
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2607.11565v1
- Score: 2.654 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 4. Evolutionary Data Through Schemaboi: Achieving Forward, Backwards, and Sideways Compatibility
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/07/durable-document-schema/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.535 | Reliability: 1.0
- Why it matters: 

## 5. MAGIC: Transition-Aware Generation of Navigable Multi-Scene Game Worlds with Large Language Models
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2607.11594v1
- Score: 2.526 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 6. [AINews] Codex usage up >10x in 6 months to 7M users, +1M in the past ~day; did Codex overtake Claude Code??
- Type: news | Source: latent_space
- URL: https://www.latent.space/p/ainews-codex-usage-up-10x-in-6-months
- Score: 2.448 | Reliability: 1.0
- Why it matters: Matches feed focus: codex, claude code.

## 7. Using uvx in GitHub Actions in a cache-friendly way
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/14/uvx-github-actions-cache/#atom-everything
- Score: 2.422 | Reliability: 1.0
- Why it matters: 

## 8. How sales teams use ChatGPT Work
- Type: news | Source: openai_blog
- URL: https://openai.com/academy/codex-for-work/how-sales-teams-use-codex
- Score: 2.363 | Reliability: 1.0
- Why it matters: 

## 9. How data science teams use ChatGPT Work
- Type: news | Source: openai_blog
- URL: https://openai.com/academy/codex-for-work/how-data-science-teams-use-codex
- Score: 2.194 | Reliability: 1.0
- Why it matters: 

## 10. Empowering India’s next generation of innovators with ATL Saathi
- Type: news | Source: google_deepmind_blog
- URL: https://deepmind.google/blog/empowering-indias-next-generation-of-innovators-with-atl-saathi/
- Score: 2.102 | Reliability: 1.0
- Why it matters: 

## 11. claude-code v2.1.208
- Type: release | Source: claude_code_releases
- URL: https://github.com/anthropics/claude-code/releases/tag/v2.1.208
- Score: 2.412 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, claude code.

## 12. codex 0.144.4
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.144.4
- Score: 2.379 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 13. DOOMQL
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/13/doomql/#atom-everything
- Score: 2.377 | Reliability: 1.0
- Why it matters: 

## 14. claude-agent-sdk-python v0.2.118
- Type: release | Source: claude_agent_sdk_python_releases
- URL: https://github.com/anthropics/claude-agent-sdk-python/releases/tag/v0.2.118
- Score: 2.312 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 15. Tool-Adaptive LLM Reranker
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2607.10555v1
- Score: 2.279 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation.

## 16. How Claude's values vary by model and language
- Type: research | Source: anthropic_research
- URL: https://www.anthropic.com/research/claude-values-models-languages
- Score: 2.278 | Reliability: 1.0
- Why it matters: 

## 17. vllm v0.25.1
- Type: release | Source: vllm_releases
- URL: https://github.com/vllm-project/vllm/releases/tag/v0.25.1
- Score: 2.197 | Reliability: 1.0
- Why it matters: 

## 18. Working at the frontier: How Hebbia builds AI for financial diligence that can't miss a detail | Claude by Anthropic
- Type: news | Source: claude_blog
- URL: https://claude.com/blog/working-at-the-frontier-how-hebbia-builds-ai-for-financial-diligence-that-cant-miss-a-detail
- Score: 2.053 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 19. EAGLE-3 Speculative Decoding on AMD Instinct GPUs: Training and Serving with vLLM and AMD Quark
- Type: news | Source: vllm_blog
- URL: https://vllm.ai/blog/2026-07-13-eagle-3-amd-instinct
- Score: 1.901 | Reliability: 1.0
- Why it matters: 

## 20. 6 months to live for open models
- Type: news | Source: interconnects
- URL: https://www.interconnects.ai/p/6-months-to-live-for-open-models
- Score: 1.818 | Reliability: 1.0
- Why it matters: 

## 21. When your brain works differently, AI isn’t a luxury—it’s accessibility
- Type: news | Source: aws_ml_blog
- URL: https://aws.amazon.com/blogs/machine-learning/when-your-brain-works-differently-ai-isnt-a-luxury-its-accessibility/
- Score: 1.107 | Reliability: 1.0
- Why it matters:

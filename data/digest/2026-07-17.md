# Daily AI SOTA Digest - 2026-07-17

Focus: AI Platform Engineering

## 1. QCon AI Boston: Production AI Moves Beyond Prompts to Platforms, Harnesses, and Evals
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/07/production-ai-platforms-evals/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.862 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, harness, eval.

## 2. 13 hands-on demos to build on Gemini Enterprise Agent Platform
- Type: news | Source: google_cloud_blog
- URL: https://cloud.google.com/blog/products/ai-machine-learning/13-demos-on-gemini-enterprise-agent-platform/
- Score: 2.825 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, evaluation, codex.

## 3. Show HN: Sentinel – open-source QA agent that reads your code before it clicks
- Type: news | Source: hackernews_ai
- URL: https://news.ycombinator.com/item?id=48952249
- Score: 2.676 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 4. Cloud Native Infrastructure Emerges as the Foundation for Trustworthy Agentic AI
- Type: news | Source: infoq_ai_ml
- URL: https://www.infoq.com/news/2026/07/cncf-trustworthy-agentic-ai/?utm_campaign=infoq_content&utm_source=infoq&utm_medium=feed&utm_term=AI%2C+ML+%26+Data+Engineering
- Score: 2.623 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 5. LQCDMaster: Agentic Scientific Computing for Lattice Quantum Chromodynamics Research
- Type: paper | Source: arxiv_cs_ai
- URL: http://arxiv.org/abs/2607.15001v1
- Score: 2.533 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic, eval.

## 6. How Cursor knew Claude Fable 5 was ready for the hardest 1% of problems | Claude by Anthropic
- Type: news | Source: claude_blog
- URL: https://claude.com/blog/working-at-the-frontier-cursor
- Score: 2.46 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

## 7. Quoting Kimi K3
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/17/kimi-k3/#atom-everything
- Score: 2.415 | Reliability: 1.0
- Why it matters: 

## 8. A scorecard for the AI age
- Type: news | Source: openai_blog
- URL: https://openai.com/index/a-scorecard-for-the-ai-age
- Score: 2.385 | Reliability: 1.0
- Why it matters: 

## 9. Digital Pantheon: Simulating and Auditing Coalition Formation with LLM Agents
- Type: paper | Source: arxiv_llm_reliability
- URL: http://arxiv.org/abs/2607.15095v1
- Score: 2.377 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, eval.

## 10. Silicon Valley Must Think Smarter About China's New AI Shock - Bloomberg.com
- Type: news | Source: search_cn_open_weight_labs
- URL: https://news.google.?oc=5
- Score: 2.365 | Reliability: 1.0
- Why it matters: 

## 11. codex 0.145.0-alpha.22
- Type: release | Source: openai_codex_releases
- URL: https://github.com/openai/codex/releases/tag/rust-v0.145.0-alpha.22
- Score: 2.357 | Reliability: 1.0
- Why it matters: Matches feed focus: codex.

## 12. [AINews] Kimi K3 2.8T-A50B: the largest open model ever released; Opus 4.8-class at Sonnet 5 pricing
- Type: release | Source: latent_space
- URL: https://www.latent.space/p/ainews-kimi-k3-28t-a50b-the-largest
- Score: 2.304 | Reliability: 1.0
- Why it matters: 

## 13. Building Managed Agents That Use GitHub Without Exposing Your Token
- Type: news | Source: philschmid
- URL: https://www.philschmid.de/managed-agents-gh
- Score: 2.285 | Reliability: 1.0
- Why it matters: Matches feed focus: agent.

## 14. OmniaBench: Benchmarking General AI Agents Across Diverse Scenarios
- Type: paper | Source: arxiv_cs_cl
- URL: http://arxiv.org/abs/2607.14989v1
- Score: 2.275 | Reliability: 1.0
- Why it matters: Matches feed focus: agent, evaluation.

## 15. Firefox in WebAssembly
- Type: news | Source: simon_willison
- URL: https://simonwillison.net/2026/Jul/16/firefox-in-webassembly/#atom-everything
- Score: 2.268 | Reliability: 1.0
- Why it matters: 

## 16. DriftWorld: Fast World Modeling through Drifting
- Type: paper | Source: arxiv_cs_lg
- URL: http://arxiv.org/abs/2607.15065v1
- Score: 2.142 | Reliability: 1.0
- Why it matters: Matches feed focus: evaluation.

## 17. Fine-tune video and image models at scale with NVIDIA NeMo Automodel and 🤗 Diffusers
- Type: research | Source: huggingface_blog
- URL: https://huggingface.co/blog/nvidia/scale-diffusers-finetuning-nemo-automodel
- Score: 2.111 | Reliability: 1.0
- Why it matters: 

## 18. Transform your sales organization with Amazon Quick: your new agentic AI teammate
- Type: news | Source: aws_ml_blog
- URL: https://aws.amazon.com/blogs/machine-learning/transform-your-sales-organization-with-amazon-quick-your-new-agentic-ai-teammate/
- Score: 1.882 | Reliability: 1.0
- Why it matters: Matches feed focus: agentic.

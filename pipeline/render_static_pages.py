...
SITE_CHROME_ASSET_VERSION = "20260924-inline-subscribe"
...
NAV_UPDATES_TAG = '<script src="/nav-updates.js?v=20260910-editorial" defer></script>'
POSTHOG_CLIENT_ASSET_VERSION = "20260924-email-reader-id"
...
def render_head(
  ...
  <script defer src="/subscribe-inline.js?v={SITE_CHROME_ASSET_VERSION}"></script>
...
def render_page(
    *,
    ...
    lang: str = "en",
    alternate_links: list[tuple[str, str]] | None = None,
    language_links: list[tuple[str, str, str]] | None = None,
) -> str:
    ...
    language_actions = ""
    for locale, href, label in language_links or []:
        language_actions += (
            f'\n        <a class="site-language-action" href="{escape(href)}"'
            f' data-language-link data-language-locale="{escape(locale)}" hidden'
            f' aria-label="Read this page in {escape(label)}" title="Read in {escape(label)}">'
            f'<span aria-hidden="true">🌐</span><span>{escape(locale.upper())}</span></a>'
        )
...
<body>
  <main>
    <header class="site-chrome" data-site-section="{escape(section)}">
      ...
      {nav}
      <div class="site-actions-fallback" aria-label="Page actions">{json_link}{share_btn}{language_actions}
...
def subscribe_cta_html(
    ...
    return (
        ...
        f'data-subscribe-placement="{escape(placement)}" data-subscribe-inline>{escape(label)}</a></aside>'
    )
...
def alternate_links_for_path(
    base_url: str,
    source_path: str,
    i18n_pages: dict[str, list[dict]] | None = None,
) -> list[tuple[str, str]] | None:
    pages = (i18n_pages or {}).get(source_path) or []
    if not pages:
        return None
    english_url = f"{base_url}{source_path}"
    links = [("en", english_url)]
    links += [(str(page["locale"]), f"{base_url}{page['canonical_path']}") for page in pages]
    links.append(("x-default", english_url))
    return links


def language_links_for_path(
    source_path: str,
    i18n_pages: dict[str, list[dict]] | None = None,
) -> list[tuple[str, str, str]]:
    links: list[tuple[str, str, str]] = []
    for page in (i18n_pages or {}).get(source_path) or []:
        locale = str(page["locale"])
        label = str(page.get("language_label") or locale)
        links.append((locale, str(page["canonical_path"]), label))
    return links
...
def render_daily_pages(
    ...
    def build(recap: dict, canonical: str, trail: list[tuple[str, str]]) -> str:
        ...
        return render_page(
            ...
            alternate_links=alternate_links_for_path(base_url, source_path, i18n_pages),
            language_links=language_links_for_path(source_path, i18n_pages),
        )
...
def render_story_body(
    ...
    if covered:
        ...
        for c in covered:
            ...
            rows.append(
                f'<li><a href="{escape(href)}" target="_blank" rel="noopener" data-track="story-covered">'
...
def render_story_pages(
    ...
    storyline_of: dict[str, tuple] | None = None,
    i18n_pages: dict[str, list[dict]] | None = None,
...
WIKI_PAGE_CSS = """\
    ...
    /* ---- /map : knowledge universe (orbit view) ----
       The viewport is deliberately deep-space dark in BOTH themes — it is an
       instrument screen framed by the page, not a themed surface. All ku-*
       colors are therefore hardcoded to the dark palette. */
    .ku { margin:2rem 0 .4rem; }
    .ku-head { display:flex; flex-wrap:wrap; align-items:baseline; gap:.3rem 1.1rem; padding:0 0 .55rem; }
    .ku-head b { font-family:ui-monospace,"SFMono-Regular",monospace; font-size:.65rem; font-weight:700;
      letter-spacing:.15em; text-transform:uppercase; color:var(--accent); }
    .ku-readout { font-family:ui-monospace,"SFMono-Regular",monospace; font-size:.68rem; color:var(--fg); }
    .ku-legend { margin-left:auto; font-family:ui-monospace,"SFMono-Regular",monospace;
      font-size:.62rem; color:var(--muted); }
    .ku-stage { position:relative; height:clamp(430px,68vh,640px); overflow:hidden;
      background:#070b16; border:1px solid var(--border); border-radius:3px; }
    .ku-canvas { position:absolute; inset:0; width:100%; height:100%; display:block;
      cursor:grab; touch-action:pan-y; }
    .ku-canvas:active { cursor:grabbing; }
    .ku-canvas:focus-visible { outline:3px solid color-mix(in srgb,var(--accent) 55%,transparent); outline-offset:-3px; }
    .ku-hint { margin:.55rem 0 0; font-family:ui-monospace,"SFMono-Regular",monospace;
      font-size:.64rem; line-height:1.6; color:var(--muted); max-width:62rem; }
    .ku-tooltip { position:absolute; z-index:3; pointer-events:none; max-width:250px;
      font-family:ui-monospace,"SFMono-Regular",monospace; font-size:.66rem; line-height:1.45;
      color:#e8f0fc; background:rgba(9,13,22,.93); border:1px solid #33415c; padding:.32rem .5rem; }
    .ku-hud { position:absolute; z-index:3; left:.85rem; right:.85rem; bottom:.85rem;
      display:flex; justify-content:center; align-items:stretch; gap:.4rem; pointer-events:none; }
    .ku-hud button { pointer-events:auto; touch-action:manipulation; cursor:pointer;
      color:#e8f0fc; background:rgba(10,14,23,.88); border:1px solid #33415c; border-radius:2px; }
    .ku-hud button:hover { border-color:#8fd3ff; }
    .ku-step { flex:none; width:2.4rem; font-size:1.3rem; line-height:1; }
    .ku-step:hover { color:#8fd3ff; }
    .ku-card { flex:0 1 auto; min-width:0; max-width:min(430px,100%); text-align:left;
      white-space:normal; padding:.5rem .75rem .55rem; display:flex; flex-direction:column; gap:.2rem; }
    .ku-card-name { display:flex; align-items:center; gap:.45rem; min-width:0;
      font-family:ui-monospace,"SFMono-Regular",monospace; font-size:.72rem; font-weight:700;
      letter-spacing:.09em; text-transform:uppercase; }
    .ku-card-name .ku-chip { width:.6rem; height:.6rem; }
    .ku-card-meta { font-family:ui-monospace,"SFMono-Regular",monospace; font-size:.62rem; color:#9aa6b6; }
    .ku-card-sum { font-size:.74rem; line-height:1.45; color:#b9c4d6; overflow:hidden;
      display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; }
    .ku-panel { position:absolute; z-index:4; top:.85rem; right:.85rem; bottom:.85rem;
      width:min(345px,calc(100% - 1.7rem)); overflow:auto; overscroll-behavior:contain;
      color:#e8f0fc; background:rgba(10,14,23,.94); border:1px solid #33415c; border-radius:2px;
      backdrop-filter:blur(6px); -webkit-backdrop-filter:blur(6px); }
    .ku-panel header { position:sticky; top:0; z-index:1; display:flex; align-items:center; gap:.6rem;
    ...
    .ku-close { margin-left:auto; flex:none; width:1.8rem; height:1.8rem; font-size:1rem; cursor:pointer;
      color:#e8f0fc; background:none; border:1px solid #33415c; border-radius:2px; }
    .ku-close:hover { border-color:#8fd3ff; color:#8fd3ff; }
    .ku-rows { list-style:none; margin:0; padding:.35rem .85rem .5rem; }
    .ku-row { display:flex; gap:.55rem; padding:.55rem 0; border-bottom:1px solid #1c2436; }
    .ku-row:last-child { border-bottom:0; }
    .ku-row-solution { padding-left:1.15rem; }
    .ku-dot { flex:none; width:.5rem; height:.5rem; border-radius:50%; margin-top:.5rem; }
    .ku-dot-unread { background:#ffe9bd; box-shadow:0 0 8px #ffd97a; }
    .ku-dot-read { background:#3a465c; }
    .ku-kind { display:block; font-family:ui-monospace,"SFMono-Regular",monospace; font-size:.56rem;
      font-weight:700; letter-spacing:.13em; text-transform:uppercase; color:#8ea0c0; margin:0 0 .1rem; }
    .ku-row a { color:#e8f0fc; font-size:.84rem; line-height:1.4; text-decoration:none; }
    .ku-row a:hover { color:#8fd3ff; text-decoration:underline; }
    .ku-row-meta { display:flex; align-items:center; gap:.5rem; margin-top:.2rem;
      font-family:ui-monospace,"SFMono-Regular",monospace; font-size:.6rem; color:#7f8ca3; }
    .ku-badge { font-weight:700; font-size:.56rem; letter-spacing:.1em; padding:.08rem .3rem;
      border:1px solid; border-radius:2px; }
    .ku-badge-new { color:#ffd97a; border-color:#8a6c2f; }
    ...
    @media (max-width:620px) {
      ...
      .ku-hud { left:.6rem; right:.6rem; bottom:.6rem; }
      /* the bottom-sheet panel covers the stepper — one control at a time */
      .ku-has-panel .ku-hud { display:none; }
    }
    @media (prefers-reduced-motion:reduce) { * { scroll-behavior:auto !important; } }
"""
...
def wiki_topic_hero(node: dict) -> str:
    ...
    kicker = "Agent engineering · know-how"
    return (
    ...
    for slug, node in nodes.items():
        ...
        html = render_page(
            ...
            json_ld=[
                ...
                breadcrumb_node(
                    base_url,
                    [("Home", "/"), ("Agent Know-How", "/map"), (title, f"/topic/{slug}")],
                ),
            ],
            alternate_links=alternate_links_for_path(base_url, source_path, i18n_pages),
            language_links=language_links_for_path(source_path, i18n_pages),
...
def wiki_universe_data(wiki: dict) -> dict:
    """Slim projection of the wiki graph for the /map orbit view (planets =
    areas, surface lights = topic pages). Embedded as a JSON data island so the
    static page needs no API call; web/universe/universe.js reads it."""
    nodes = wiki.get("nodes") or {}
    slim_nodes = {}
    for slug, node in nodes.items():
        if not SLUG_RE.match(slug):
        ...
        slim_nodes[slug] = {
            "title": squeeze(node.get("title")) or slug,
            "kind": node.get("kind"),
            "updated": (iso_or_none(node.get("updated")) or str(node.get("updated") or ""))[:10] or None,
            "solutions": [
                s["slug"]
                for s in (node.get("solutions") or [])
                if s.get("slug") in nodes
            ],
        }
    areas = []
    for area in wiki.get("areas") or []:
        obstacles = [s for s in (area.get("obstacles") or []) if s in slim_nodes]
        # One-liner for the orbit view's focus caption: the area's lead
        # obstacle is what the planet *is*, so its summary describes the planet.
        lead = nodes.get(obstacles[0]) if obstacles else None
        areas.append(
            {
                "id": area.get("area"),
                "label": squeeze(area.get("label")) or area.get("area") or "",
                "summary": clip(squeeze((lead or {}).get("summary")), 190),
                "obstacles": obstacles,
            }
        )
    return {"generated_at": wiki.get("generated_at"), "areas": areas, "nodes": slim_nodes}


def wiki_universe_section(wiki: dict) -> str:
    """The orbit-view mount + data island + module loader. Hidden until the
    module boots (and stays hidden without JS/WebGL — the HTML list below is
    always the fallback). Read state lives in localStorage only."""
    payload = json.dumps(
        wiki_universe_data(wiki), ensure_ascii=False, separators=(",", ":")
    ).replace("</", "<\\/")
    return (
        '<section class="ku" id="knowledge-universe" data-knowledge-universe hidden>'
        '<div class="ku-head"><b>Orbit view</b>'
        '<span class="ku-readout" data-ku-readout></span>'
        '<span class="ku-legend">bright satellite = unread · dim = read · ✦ ping = updated recently</span></div>'
        '<div class="ku-stage"></div>'
        '<p class="ku-hint">Every planet is a problem you eventually hit in production; '
        'the satellites circling it are the pages that show how the field handles it. '
        'Tour planet by planet with ‹ › · drag to orbit · scroll or pinch to zoom · '
        'click a planet to see what lives there. '
        'What you read dims — remembered only in this browser.</p>'
        "</section>"
        f'<script type="application/json" id="knowledge-universe-data">{payload}</script>'
        '<script type="module" src="/universe/universe.js?v=20260805b"></script>'
    )
...
def wiki_map_body(wiki: dict) -> str | None:
    ...
    list_lede = (
        '<h2 class="map-list-title" id="as-a-list">The same universe, as a list</h2>'
        if blocks
        else ""
    )
    body = (
        '<div class="map">'
        '<section class="wiki-hero">'
        '<p class="wiki-kicker">Agent engineering</p>'
        '<h2 class="wiki-headline">The essential know-how for production-grade agents</h2>'
        '<p class="wiki-thesis">Everything here orbits one job: keeping an agent alive in production. '
        'Each planet is a problem you will eventually hit — memory, cost, evals, security — '
        'linked to the solutions the field actually uses. Every claim is grounded in source articles '
        'already in the feed — one shared structure, no hype.</p>'
...
def render_map_page(base_url: str, wiki: dict) -> bool:
    ...
    description = (
        "The essential know-how for production-grade agents — memory, "
        "reliability, tool use, cost, and more — every problem area linked to "
        "the solutions the field uses, with source articles."
    )
    html = render_page(
        title="Agent Know-How — the essentials for production-grade agents",
        description=description,
        canonical=canonical,
        published=None,
        h1="Agent Know-How",
        ...
        extra_css=WIKI_PAGE_CSS,
        json_ld=[
            breadcrumb_node(base_url, [("Home", "/"), ("Agent Know-How", "/map")]),
        ],
    )
...
def render_foundation_pages(
    ...
    for slug, concept in concepts.items():
        ...
        source_path = f"/foundations/{slug}"
...
def collect_i18n_pages(
    stories: dict[str, dict],
    storyline_details: list[dict],
    wiki: dict,
    foundations: dict,
) -> dict[str, list[dict]]:
    """Return fresh pre-translated artifacts grouped by English source path."""
    storylines = {
    ...
    for locale_dir in sorted(p for p in I18N_DIR.iterdir() if p.is_dir()):
        ...
        for artifact_path in sorted(locale_dir.glob("**/*.json")):
            ...
            page = {
                "locale": locale,
                "language_label": LANGUAGE_LABELS.get(locale, locale),
                "source_path": source_path,
                "canonical_path": f"/{locale}{source_path}",
                "translated_at": str(artifact.get("translated_at") or ""),
                "artifact": artifact,
            }
            grouped.setdefault(source_path, []).append(page)
    return grouped
...
def render_i18n_pages(
    ...
    for source_path, pages in sorted(i18n_pages.items()):
        for page in pages:
            locale = str(page["locale"])
            artifact = page["artifact"]
            canonical_path = str(page["canonical_path"])
...
def main() -> None:
    ...
    i18n_page_map = collect_i18n_pages(stories, storyline_details, wiki, foundations)

    days = render_daily_pages(base_url, story_sids, playbook_index, i18n_page_map)
    weeks = render_weekly_pages(base_url, story_sids, playbook_index, i18n_page_map)
    story_pages, noindexed = render_story_pages(base_url, stories, storyline_of, i18n_page_map)
    storyline_pages = render_storyline_pages(
        base_url, storyline_details, active_slugs, story_sids, active_members, i18n_page_map
    )
    topic_pages = render_topic_pages(base_url, wiki, i18n_page_map)

...
test('brief excludes inferred releases even when their heuristic category was platform', async () => {
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify([
        ...
        { id: 'release', title: 'sqlite-utils 4.1', type: 'release', llm_category: 'platform' },
      ]),
    );
    ...
    assert.deepEqual(brief.items.map((item) => item.id), ['analysis']);

...
- Vercel runs `python3 pipeline/render_static_pages.py` as its build command, so
  every preview/production deploy regenerates static story/recap/storyline
  pages from the committed data. This keeps code-only renderer PR previews
  accurate without committing 1,000+ generated files.

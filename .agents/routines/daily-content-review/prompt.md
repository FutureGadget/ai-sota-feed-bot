...
4. Verify suspected factual errors against the linked source and, when
...
Edit the recap source JSON, not generated HTML. Follow the daily-summary
skill's URL-preservation standard, including its correction exception.

Rebuild and validate per the skill's index-build step. Fix errors and repeat
until all recaps validate. Inspect the regenerated page to confirm the
correction appears correctly and introduced no placeholder or layout-breaking
content.
